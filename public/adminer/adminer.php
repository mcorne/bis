...
remove_slashes($Pf,$Lc=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($Pf)){foreach($X
as$Kd=>$W){unset($Pf[$y][$Kd]);if(is_array($W)){$Pf[$y][stripslashes($Kd)]=$W;$Pf[]=&$Pf[$y][stripslashes($Kd)];}else$Pf[$y][stripslashes($Kd)]=($Lc?$W:stripslashes($W));}}}}function
bracket_escape($u,$Na=false){static$Dh=array(':'=>':1',']'=>':2','['=>':3','"'=>':4');return
strtr($u,($Na?array_flip($Dh):$Dh));}function
...
nl_br($Q){return
str_replace("\n","<br>",$Q);}function
checkbox($C,$Y,$db,$Rd="",$Ue="",$ib="",$Sd=""){$J="<input type='checkbox' name='$C' value='".h($Y)."'".($db?" checked":"").($Sd?" aria-labelledby='$Sd'":"").($Ue?' onclick="'.h($Ue).'"':'').">";return($Rd!=""||$ib?"<label".($ib?" class='$ib'":"").">$J".h($Rd)."</label>":$J);}function
optionlist($af,$Ag=null,$bi=false){$J="";foreach($af
as$Kd=>$W){$bf=array($Kd=>$W);if(is_array($W)){$J.='<optgroup label="'.h($Kd).'">';$bf=$W;}foreach($bf
as$y=>$X)$J.='<option'.($bi||is_string($y)?' value="'.h($y).'"':'').(($bi||is_string($y)?(string)$y:$X)===$Ag?' selected':'').'>'.h($X);if(is_array($W))$J.='</optgroup>';}return$J;}function
html_select($C,$af,$Y="",$Te=true,$Sd=""){if($Te)return"<select name='".h($C)."'".(is_string($Te)?' onchange="'.h($Te).'"':"").($Sd?" aria-labelledby='$Sd'":"").">".optionlist($af,$Y)."</select>";$J="";foreach($af
as$y=>$X)$J.="<label><input type='radio' name='".h($C)."' value='".h($y)."'".($y==$Y?" checked":"").">".h($X)."</label>";return$J;}function
select_input($Ja,$af,$Y="",$Bf=""){return($af?"<select$Ja><option value=''>$Bf".optionlist($af,$Y,true)."</select>":"<input$Ja size='10' value='".h($Y)."' placeholder='$Bf'>");}function
confirm(){return" onclick=\"return confirm('".lang(0)."');\"";}function
print_fieldset($t,$ae,$mi=false,$Ue=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($Ue)."return !toggle('fieldset-$t');\">$ae</a></legend><div id='fieldset-$t'".($mi?"":" class='hidden'").">\n";}function
...
where($bb,$p);}function
where_link($s,$d,$Y,$We="="){return"&where%5B$s%5D%5Bcol%5D=".urlencode($d)."&where%5B$s%5D%5Bop%5D=".urlencode(($Y!==null?$We:"IS NULL"))."&where%5B$s%5D%5Bval%5D=".urlencode($Y);}function
...
auth_url($ii,$N,$V,$m=null){global$Yb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($Yb))."|username|".($m!==null?"db|":"").session_name()),$B);return"$B[1]?".(sid()?SID."&":"").($ii!="server"||$N!=""?urlencode($ii)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
...
query_redirect($H,$A,$re,$Zf=true,$xc=true,$Ec=false,$sh=""){global$g,$n,$b;if($xc){$Sg=microtime(true);$Ec=!$g->query($H);$sh=format_time($Sg);}$Qg="";if($H)$Qg=$b->messageQuery($H,$sh);if($Ec){$n=error().$Qg;return
false;}if($Zf)redirect($A,$re.$Qg);return
true;}function
queries($H){global$g;static$Tf=array();static$Sg;if(!$Sg)$Sg=microtime(true);if($H===null)return
array(implode("\n",$Tf),format_time($Sg));$Tf[]=(preg_match('~;$~',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H).";";return$g->query($H);}function
apply_queries($H,$T,$tc='table'){foreach($T
as$R){if(!queries("$H ".$tc($R)))return
false;}return
true;}function
queries_redirect($A,$re,$Zf){list($Tf,$sh)=queries(null);return
query_redirect($Tf,$A,$re,$Zf,false,!$Zf,$sh);}function
format_time($Sg){return
lang(1,max(0,microtime(true)-$Sg));}function
...
null;foreach($Jc
as$y=>$X)$Jc[$y]=(array)$X;$J='';foreach($Jc["error"]as$y=>$n){if($n)return$n;$C=$Jc["name"][$y];$_h=$Jc["tmp_name"][$y];$xb=file_get_contents($Lb&&preg_match('~\\.gz$~',$C)?"compress.zlib://$_h":$_h);if($Lb){$Sg=substr($xb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Sg,$fg))$xb=iconv("utf-16","utf-8",$xb);elseif($Sg=="\xEF\xBB\xBF")$xb=substr($xb,3);$J.=$xb."\n\n";}else$J.=$xb;}return$J;}function
upload_error($n){$oe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($oe?" ".lang(3,$oe):""):lang(4));}function
repeat_pattern($_f,$be){return
str_repeat("$_f{0,65535}",$be/65535)."$_f{0,".($be%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($Q,$be=80,$Yg=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{10FFFF}]",$be).")($)?)u",$Q,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$be).")($)?)",$Q,$B);return
h($B[1]).$Yg.(isset($B[2])?"":"<i>...</i>");}function
format_number($X){return
strtr(number_format($X,0,".",lang(5)),preg_split('~~u',lang(6),-1,PREG_SPLIT_NO_EMPTY));}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($Pf,$qd=array()){while(list($y,$X)=each($Pf)){if(!in_array($y,$qd)){if(is_array($X)){foreach($X
as$Kd=>$W)$Pf[$y."[$Kd]"]=$W;}else
...
enum_input($U,$Ja,$o,$Y,$nc=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$je);$J=($nc!==null?"<label><input type='$U'$Ja value='$nc'".((is_array($Y)?in_array($nc,$Y):$Y===0)?" checked":"")."><i>".lang(7)."</i></label>":"");foreach($je[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$db=(is_int($Y)?$Y==$s+1:(is_array($Y)?in_array($s+1,$Y):$Y===$X));$J.=" <label><input type='$U'$Ja value='".($s+1)."'".($db?' checked':'').'>'.h($b->editVal($X,$o)).'</label>';}return$J;}function
input($o,$Y,$r){global$g,$Oh,$b,$x;$C=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$r){$Ea=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$Ea[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$Ea);$r="json";}$ig=($x=="mssql"&&$o["auto_increment"]);if($ig&&!$_POST["save"])$r=null;$Zc=(isset($_GET["select"])||$ig?array("orig"=>lang(8)):array())+$b->editFunctions($o);$Ja=" name='fields[$C]'";if($o["type"]=="enum")echo
nbsp($Zc[""])."<td>".$b->editInput($_GET["edit"],$o,$Ja,$Y);else{$Mc=0;foreach($Zc
as$y=>$X){if($y===""||!$X)break;$Mc++;}$Te=($Mc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($Mc > f.selectedIndex) f.selectedIndex = $Mc;\" onkeyup='keyupChange.call(this);'":"");$Ja.=$Te;$hd=(in_array($r,$Zc)||isset($Zc[$r]));echo(count($Zc)>1?"<select name='function[$C]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($Zc,$r===null||$hd?$r:"")."</select>":nbsp(reset($Zc))).'<td>';$zd=$b->editInput($_GET["edit"],$o,$Ja,$Y);if($zd!="")echo$zd;elseif(preg_match('~bool~',$o["type"]))echo"<input type='hidden'$Ja value='0'>"."<input type='checkbox'".(in_array(strtolower($Y),array('1','t','true','y','yes','on'))?" checked='checked'":"")."$Ja value='1'>";elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$je);foreach($je[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$db=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$C][$s]' value='".(1<<$s)."'".($db?' checked':'')."$Te>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$C'$Te>";elseif(($qh=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($qh&&$x!="sqlite")$Ja.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$Ja.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$Ja>".h($Y).'</textarea>';}elseif($r=="json"||preg_match('~^jsonb?$~',$o["type"]))echo"<textarea$Ja cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$qe=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$B)?((preg_match("~binary~",$o["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$o["unsigned"]?1:0)):($Oh[$o["type"]]?$Oh[$o["type"]]+($o["unsigned"]?0:1):0));if($x=='sql'&&$g->server_info>=5.6&&preg_match('~time~',$o["type"]))$qe+=7;echo"<input".((!$hd||$r==="")&&preg_match('~(?<!o)int~',$o["type"])&&!preg_match('~\[\]~',$o["full_type"])?" type='number'":"")." value='".h($Y)."'".($qe?" data-maxlength='$qe'":"").(preg_match('~char|binary~',$o["type"])&&$qe>20?" size='40'":"")."$Ja>";}echo$b->editHint($_GET["edit"],$o,$Y);}}function
...
dump_headers($od,$_e=false){global$b;$J=$b->dumpHeaders($od,$_e);$nf=$_POST["output"];if($nf!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($od).".$J".($nf!="file"&&!preg_match('~[^0-9a-z]~',$nf)?".$nf":""));session_write_close();ob_flush();flush();return$J;}function
...
slow_query($H){global$b,$Ah;$m=$b->database();$th=$b->queryTimeout();if(support("kill")&&is_object($h=connect())&&($m==""||$h->select_db($m))){$Pd=$h->result(connection_id());echo'<script type="text/javascript">
...
select($R,$M,$Z,$cd,$cf=array(),$z=1,$E=0,$Lf=false){global$b,$x;$Ed=(count($cd)<count($M));$H=$b->selectQueryBuild($M,$Z,$cd,$cf,$z,$E);if(!$H)$H="SELECT".limit(($_GET["page"]!="last"&&+$z&&$cd&&$Ed&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$M)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($cd&&$Ed?"\nGROUP BY ".implode(", ",$cd):"").($cf?"\nORDER BY ".implode(", ",$cf):""),($z!=""?+$z:null),($E?$z*$E:0),"\n");$Sg=microtime(true);$J=$this->_conn->query($H);if($Lf)echo$b->selectQuery($H,format_time($Sg));return$J;}function
delete($R,$Uf,$z=0){$H="FROM ".table($R);return
queries("DELETE".($z?limit1($H,$Uf):" $H$Uf"));}function
update($R,$O,$Uf,$z=0,$Cg="\n"){$gi=array();foreach($O
as$y=>$X)$gi[]="$y = $X";$H=table($R)." SET$Cg".implode(",$Cg",$gi);return
...
SQLite3($Kc);$ji=$this->_link->version();$this->server_info=$ji["versionString"];$this->_link->querySingle('PRAGMA foreign_keys = ON');}function
...
query($H,$Ph=false){$xe=($Ph?"unbufferedQuery":"query");$I=@$this->_link->$xe($H,SQLITE_BOTH,$n);$this->error="";if(!$I){$this->error=$n;return
...
extends
Min_SQL{function
insertUpdate($R,$L,$Jf){$gi=array();foreach($L
...
array();}function
limit($H,$Z,$z,$D=0,$Cg=" "){return" $H$Z".($z!==null?$Cg."LIMIT $z".($D?" OFFSET $D":""):"");}function
...
fields($R){global$g;$J=array();$Jf="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$K){$C=$K["name"];$U=strtolower($K["type"]);$Mb=$K["dflt_value"];$J[$C]=array("field"=>$C,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$Mb,$B)?str_replace("''","'",$B[1]):($Mb=="NULL"?null:$Mb)),"null"=>!$K["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$K["pk"],);if($K["pk"]){if($Jf!="")$J[$Jf]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$J[$C]["auto_increment"]=true;$Jf=$C;}}$Qg=$g->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Qg,$je,PREG_SET_ORDER);foreach($je
as$B){$C=str_replace('""','"',preg_replace('~^"|"$~','',$B[1]));if($J[$C])$J[$C]["collation"]=trim($B[3],"'");}return$J;}function
indexes($R,$h=null){global$g;if(!is_object($h))$h=$g;$J=array();$Qg=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Qg,$B)){$J[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$B[1],$je,PREG_SET_ORDER);foreach($je
as$B){$J[""]["columns"][]=idf_unescape($B[2]).$B[4];$J[""]["descs"][]=(preg_match('~DESC~i',$B[5])?'1':null);}}if(!$J){foreach(fields($R)as$C=>$o){if($o["primary"])$J[""]=array("type"=>"PRIMARY","columns"=>array($C),"lengths"=>array(),"descs"=>array(null));}}$Rg=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$h);foreach(get_rows("PRAGMA index_list(".table($R).")",$h)as$K){$C=$K["name"];$v=array("type"=>($K["unique"]?"UNIQUE":"INDEX"));$v["lengths"]=array();$v["descs"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($C).")",$h)as$tg){$v["columns"][]=$tg["name"];$v["descs"][]=null;}if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($C).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$Rg[$C],$fg)){preg_match_all('/("[^"]*+")+( DESC)?/',$fg[2],$je);foreach($je[2]as$y=>$X){if($X)$v["descs"][$y]='1';}}if(!$J[""]||$v["type"]!="UNIQUE"||$v["columns"]!=$J[""]["columns"]||$v["descs"]!=$J[""]["descs"]||!preg_match("~^sqlite_~",$C))$J[$C]=$v;}return$J;}function
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){$ai=($R==""||$Rc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$ai=true;break;}}$c=array();$lf=array();foreach($p
as$o){if($o[1]){$c[]=($ai?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$lf[$o[0]]=$o[1][0];}}if(!$ai){foreach($c
...
true;}function
recreate_table($R,$C,$p,$lf,$Rc,$w=array()){if($R!=""){if(!$p){foreach(fields($R)as$y=>$o){$p[]=process_field($o,$o);$lf[$y]=idf_escape($y);}}$Kf=false;foreach($p
as$o){if($o[6])$Kf=true;}$bc=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$bc[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$Nd=>$v){$e=array();foreach($v["columns"]as$y=>$d){if(!$lf[$d])continue
2;$e[]=$lf[$d].($v["descs"][$y]?" DESC":"");}if(!$bc[$Nd]){if($v["type"]!="PRIMARY"||!$Kf)$w[]=array($v["type"],$Nd,$e);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$Rc[]="  PRIMARY KEY (".implode(", ",$X[2]).")";}}foreach(foreign_keys($R)as$Nd=>$q){foreach($q["source"]as$y=>$d){if(!$lf[$d])continue
2;$q["source"][$y]=idf_unescape($lf[$d]);}if(!isset($Rc[" $Nd"]))$Rc[]=" ".format_foreign_key($q);}queries("BEGIN");}foreach($p
...
array("Statement"=>"BEGIN\n\t;\nEND");$u='(?:[^`"\\s]+|`[^`]*`|"[^"]*")+';$Kh=trigger_options();preg_match("~^CREATE\\s+TRIGGER\\s*$u\\s*(".implode("|",$Kh["Timing"]).")\\s+([a-z]+)(?:\\s+OF\\s+($u))?\\s+ON\\s*$u\\s*(?:FOR\\s+EACH\\s+ROW\\s)?(.*)~is",$g->result("SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = ".q($C)),$B);$Le=$B[3];return
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$Ze){list($y,$X)=explode("=",$Ze,2);$J[$y]=$X;}return$J;}function
convert_field($o){}function
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|dump|indexes|move_col|sql|status|table|trigger|variables|view|view_trigger)$~',$Gc);}$x="sqlite";$Oh=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$Vg=array_keys($Oh);$Vh=array();$Xe=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","SQL");$Zc=array("hex","length","lower","round","unixepoch","upper");$ed=array("avg","count","count distinct","group_concat","max","min","sum");$gc=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$Yb["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$Gf=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
extends
Min_SQL{function
insertUpdate($R,$L,$Jf){global$g;foreach($L
as$O){$Wh=array();$Z=array();foreach($O
as$y=>$X){$Wh[]="$y = $X";if(isset($Jf[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$Wh)." WHERE ".implode(" AND ",$Z))&&$g->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")")))return
false;}return
true;}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
...
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2])){if($g->server_info>=9){$g->query("SET application_name = 'Adminer'");if($g->server_info>=9.2){$Vg[lang(23)][]="json";$Oh["json"]=4294967295;if($g->server_info>=9.4){$Vg[lang(23)][]="jsonb";$Oh["jsonb"]=4294967295;}}}return$g;}return$g->error;}function
get_databases(){return
...
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$B)){$K['source']=array_map('trim',explode(',',$B[1]));if(preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~',$B[2],$ie)){$K['ns']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$ie[2]));$K['table']=str_replace('""','"',preg_replace('~^"(.+)"$~','\1',$ie[4]));}$K['target']=array_map('trim',explode(',',$B[3]));$K['on_delete']=(preg_match("~ON DELETE ($Se)~",$B[4],$ie)?$ie[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($Se)~",$B[4],$ie)?$ie[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($C){global$g;return
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){$c=array();$Tf=array();foreach($p
as$o){$d=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $d";else{$fi=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($d!=$X[0])$Tf[]="ALTER TABLE ".table($R)." RENAME $d TO $X[0]";$c[]="ALTER $d TYPE$X[1]";if(!$X[6]){$c[]="ALTER $d ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $d ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$fi!="")$Tf[]="COMMENT ON COLUMN ".table($R).".$X[0] IS ".($fi!=""?substr($fi,9):"''");}}$c=array_merge($c,$Rc);if($R=="")array_unshift($Tf,"CREATE TABLE ".table($C)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($Tf,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""&&$R!=$C)$Tf[]="ALTER TABLE ".table($R)." RENAME TO ".table($C);if($R!=""||$sb!="")$Tf[]="COMMENT ON TABLE ".table($C)." IS ".q($sb);if($La!=""){}foreach($Tf
as$H){if(!queries($H))return
false;}return
true;}function
alter_indexes($R,$c){$i=array();$Zb=array();$Tf=array();foreach($c
as$X){if($X[0]!="INDEX")$i[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"")."(".implode(", ",$X[2]).")");elseif($X[2]=="DROP")$Zb[]=idf_escape($X[1]);else$Tf[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R)." (".implode(", ",$X[2]).")";}if($i)array_unshift($Tf,"ALTER TABLE ".table($R).implode(",",$i));if($Zb)array_unshift($Tf,"DROP INDEX ".implode(", ",$Zb));foreach($Tf
as$H){if(!queries($H))return
false;}return
true;}function
truncate_tables($T){return
...
found_rows($S,$Z){global$g;if(preg_match("~ rows=([0-9]+)~",$g->result("EXPLAIN SELECT * FROM ".idf_escape($S["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$fg))return$fg[1];return
false;}function
types(){return
...
set_schema($wg){global$g,$Oh,$Vg;$J=$g->query("SET search_path TO ".idf_escape($wg));foreach(types()as$U){if(!isset($Oh[$U])){$Oh[$U]=0;$Vg[lang(24)][]=$U;}}return$J;}function
create_sql($R,$La){global$g;$J='';$mg=array();$Eg=array();$P=table_status($R);$p=fields($R);$w=indexes($R);ksort($w);$Pc=foreign_keys($R);ksort($Pc);$Lh=triggers($R);if(!$P||empty($p))return
false;$J="CREATE TABLE ".idf_escape($P['nspname']).".".idf_escape($P['Name'])." (\n    ";foreach($p
as$Ic=>$o){$sf=idf_escape($o['field']).' '.$o['full_type'].(is_null($o['default'])?"":" DEFAULT $o[default]").($o['attnotnull']?" NOT NULL":"");$mg[]=$sf;if(preg_match('~nextval\(\'([^\']+)\'\)~',$o['default'],$je)){$Dg=$je[1];$Pg=reset(get_rows("SELECT * FROM $Dg"));$Eg[]="CREATE SEQUENCE $Dg INCREMENT $Pg[increment_by] MINVALUE $Pg[min_value] MAXVALUE $Pg[max_value] START ".($La?$Pg['last_value']:1)." CACHE $Pg[cache_value];";}}if(!empty($Eg))$J=implode("\n\n",$Eg)."\n\n$J";foreach($w
as$sd=>$v){switch($v['type']){case'UNIQUE':$mg[]="CONSTRAINT ".idf_escape($sd)." UNIQUE (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;case'PRIMARY':$mg[]="CONSTRAINT ".idf_escape($sd)." PRIMARY KEY (".implode(', ',array_map('idf_escape',$v['columns'])).")";break;}}foreach($Pc
...
as$Ic=>$o){if($o['comment'])$J.="\n\nCOMMENT ON COLUMN ".idf_escape($P['nspname']).".".idf_escape($P['Name']).".".idf_escape($Ic)." IS ".q($o['comment']).";";}foreach($Lh
...
limit($H,$Z,$z,$D=0,$Cg=" "){return($D?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $H$Z) t WHERE rownum <= ".($z+$D).") WHERE rnum > $D":($z!==null?" * FROM (SELECT $H$Z) WHERE rownum <= ".($z+$D):" $H$Z"));}function
limit1($H,$Z){return" $H$Z";}function
...
found_rows($S,$Z){}function
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){$c=$Zb=array();foreach($p
...
set_schema($xg){global$g;return$g->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($xg));}function
show_variables(){return
...
unconvert_field($o,$J){return$J;}function
support($Gc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Gc);}$x="oracle";$Oh=array();$Vg=array();foreach(array(lang(25)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(26)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(23)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(27)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$Oh+=$X;$Vg[$y]=array_keys($X);}$Vh=array();$Xe=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Zc=array("length","lower","round","upper");$ed=array("avg","count","count distinct","max","min","sum");$gc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Yb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Gf=array("SQLSRV","MSSQL","PDO_DBLIB");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
as$O){$Wh=array();$Z=array();foreach($O
as$y=>$X){$Wh[]="$y = $X";if(isset($Jf[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$Wh)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");"))return
false;}return
true;}function
begin(){return
...
limit($H,$Z,$z,$D=0,$Cg=" "){return($z!==null?" TOP (".($z+$D).")":"")." $H$Z";}function
limit1($H,$Z){return
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){$c=array();foreach($p
...
limit($H,$Z,$z,$D=0,$Cg=" "){$J='';$J.=($z!==null?$Cg."FIRST $z".($D?" SKIP $D":""):"");$J.=" $H$Z";return$J;}function
limit1($H,$Z){return
limit($H,$Z,1);}function
...
true;}function
support($Gc){return
preg_match("~^(columns|sql|status|table)$~",$Gc);}$x="firebird";$Xe=array("=");$Zc=array();$ed=array();$gc=array();}$Yb["simpledb"]="SimpleDB";if(isset($_GET["simpledb"])){$Gf=array("SimpleXML");define("DRIVER","simpledb");if(class_exists('SimpleXMLElement')){class
...
as$Id)$Zg+=$Id->Attribute->Value;$I=array((object)array('Attribute'=>array((object)array('Name'=>'Count','Value'=>$Zg,))));}return
new
Min_Result($I);}function
multi_query($H){return$this->_result=$this->query($H);}function
store_result(){return$this->_result;}function
...
_chunkRequest($pd,$va,$F,$zc=array()){global$g;foreach(array_chunk($pd,25)as$gb){$qf=$F;foreach($gb
as$s=>$t){$qf["Item.$s.ItemName"]=$t;foreach($zc
as$y=>$X)$qf["Item.$s.$y"]=$X;}if(!sdb_request($va,$qf))return
false;}$g->affected_rows=count($pd);return
true;}function
_extractIds($R,$Uf,$z){$J=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$Uf,$je))$J=array_map('idf_unescape',$je[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$Uf.($z?" LIMIT 1":"")))as$Id)$J[]=$Id->Name;}return$J;}function
select($R,$M,$Z,$cd,$cf=array(),$z=1,$E=0,$Lf=false){global$g;$g->next=$_GET["next"];$J=parent::select($R,$M,$Z,$cd,$cf,$z,$E,$Lf);$g->next=0;return$J;}function
delete($R,$Uf,$z=0){return$this->_chunkRequest($this->_extractIds($R,$Uf,$z),'BatchDeleteAttributes',array('DomainName'=>$R));}function
update($R,$O,$Uf,$z=0,$Cg="\n"){$Nb=array();$_d=array();$s=0;$pd=$this->_extractIds($R,$Uf,$z);$t=idf_unescape($O["`itemName()`"]);unset($O["`itemName()`"]);foreach($O
...
table_status($C="",$Fc=false){$J=array();foreach(($C!=""?array($C=>true):tables_list())as$R=>$U){$K=array("Name"=>$R,"Auto_increment"=>"");if(!$Fc){$we=sdb_request('DomainMetadata',array('DomainName'=>$R));if($we){foreach(array("Rows"=>"ItemCount","Data_length"=>"ItemNamesSizeBytes","Index_length"=>"AttributeValuesSizeBytes","Data_free"=>"AttributeNamesSizeBytes",)as$y=>$X)$K[$y]=(string)$we->$X;}}if($C!="")return$K;$J[$R]=$K;}return$J;}function
...
idf_escape($u);}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
limit($H,$Z,$z,$D=0,$Cg=" "){return" $H$Z".($z!==null?$Cg."LIMIT $z":"");}function
unconvert_field($o,$J){return$J;}function
fk_support($S){}function
engines(){return
array();}function
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$C)));}function
drop_tables($T){foreach($T
...
last_id(){}function
hmac($Ba,$Gb,$y,$Yf=false){$Ua=64;if(strlen($y)>$Ua)$y=pack("H*",$Ba($y));$y=str_pad($y,$Ua,"\0");$Ld=$y^str_repeat("\x36",$Ua);$Md=$y^str_repeat("\x5C",$Ua);$J=$Ba($Md.pack("H*",$Ba($Ld.$Gb)));if($Yf)$J=pack("H*",$J);return$J;}function
sdb_request($va,$F=array()){global$b,$g;list($md,$F['AWSAccessKeyId'],$zg)=$b->credentials();$F['Action']=$va;$F['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$F['Version']='2009-04-15';$F['SignatureVersion']=2;$F['SignatureMethod']='HmacSHA1';ksort($F);$H='';foreach($F
as$y=>$X)$H.='&'.rawurlencode($y).'='.rawurlencode($X);$H=str_replace('%7E','~',substr($H,1));$H.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$md)."\n/\n$H",$zg,true)));@ini_set('track_errors',1);$Jc=@file_get_contents((preg_match('~^https?://~',$md)?$md:"http://$md"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$H,'ignore_errors'=>1,))));if(!$Jc){$g->error=$php_errormsg;return
false;}libxml_use_internal_errors(true);$ri=simplexml_load_string($Jc);if(!$ri){$n=libxml_get_last_error();$g->error=$n->message;return
false;}if($ri->Errors){$n=$ri->Errors->Error;$g->error="$n->Message ($n->Code)";return
false;}$g->error='';$kh=$va."Result";return($ri->$kh?$ri->$kh:true);}function
sdb_request_all($va,$kh,$F=array(),$th=0){$J=array();$Sg=($th?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$F['SelectExpression'],$B)?$B[1]:0);do{$ri=sdb_request($va,$F);if(!$ri)break;foreach($ri->$kh
as$jc)$J[]=$jc;if($z&&count($J)>=$z){$_GET["next"]=$ri->NextToken;break;}if($th&&microtime(true)-$Sg>$th)return
false;$F['NextToken']=$ri->NextToken;if($z)$F['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($J),$F['SelectExpression']);}while($ri->NextToken);return$J;}$x="simpledb";$Xe=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$Zc=array();$ed=array("count");$gc=array(array("json"));}$Yb["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$Gf=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
Min_DB{var$extension="Mongo",$error,$last_id,$_link,$_db;function
connect($N,$V,$G){global$b;$m=$b->database();$af=array();if($V!=""){$af["username"]=$V;$af["password"]=$G;}if($m!="")$af["db"]=$m;try{$this->_link=@new
...
select($R,$M,$Z,$cd,$cf=array(),$z=1,$E=0,$Lf=false){$M=($M==array("*")?array():array_fill_keys($M,true));$Mg=array();foreach($cf
as$X){$X=preg_replace('~ DESC$~','',$X,1,$Bb);$Mg[$X]=($Bb?-1:1);}return
new
...
alter_table($R,$C,$p,$Rc,$sb,$oc,$nb,$La,$vf){global$g;if($R==""){$g->_db->createCollection($C);return
true;}}function
...
support($Gc){return
preg_match("~database|indexes~",$Gc);}$x="mongo";$Xe=array("=");$Zc=array();$ed=array();$gc=array(array("json"));}$Yb["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$Gf=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
rootQuery($yf,$xb=array(),$xe='GET'){@ini_set('track_errors',1);$Jc=@file_get_contents($this->_url.'/'.ltrim($yf,'/'),false,stream_context_create(array('http'=>array('method'=>$xe,'content'=>json_encode($xb),'ignore_errors'=>1,))));if(!$Jc){$this->error=$php_errormsg;return$Jc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$http_response_header[0])){$this->error=$Jc;return
...
query($yf,$xb=array(),$xe='GET'){return$this->rootQuery(($this->_db!=""?"$this->_db/":"/").ltrim($yf,'/'),$xb,$xe);}function
...
extends
Min_SQL{function
select($R,$M,$Z,$cd,$cf=array(),$z=1,$E=0,$Lf=false){global$b;$Gb=array();$H="$R/_search";if($M!=array("*"))$Gb["fields"]=$M;if($cf){$Mg=array();foreach($cf
as$lb){$lb=preg_replace('~ DESC$~','',$lb,1,$Bb);$Mg[]=($Bb?array($lb=>"desc"):$lb);}$Gb["sort"]=$Mg;}if($z){$Gb["size"]=+$z;if($E)$Gb["from"]=($E*$z);}foreach($Z
as$X){list($lb,$Ve,$X)=explode(" ",$X,3);if($lb=="_id")$Gb["query"]["ids"]["values"][]=$X;elseif($lb.$X!=""){$oh=array("term"=>array(($lb!=""?$lb:"_all")=>$X));if($Ve=="=")$Gb["query"]["filtered"]["filter"]["and"][]=$oh;else$Gb["query"]["filtered"]["query"]["bool"]["must"][]=$oh;}}if($Gb["query"]&&!$Gb["query"]["filtered"]["query"]&&!$Gb["query"]["ids"])$Gb["query"]["filtered"]["query"]=array("match_all"=>array());$Sg=microtime(true);$yg=$this->_conn->query($H,$Gb);if($Lf)echo$b->selectQuery("$H: ".print_r($Gb,true),format_time($Sg));if(!$yg)return
false;$J=array();foreach($yg['hits']['hits']as$ld){$K=array();if($M==array("*"))$K["_id"]=$ld["_id"];$p=$ld['_source'];if($M!=array("*")){$p=array();foreach($M
...
table_status($C="",$Fc=false){global$g;$yg=$g->query("_search?search_type=count",array("facets"=>array("count_by_type"=>array("terms"=>array("field"=>"_type",)))),"POST");$J=array();if($yg){foreach($yg["facets"]["count_by_type"]["terms"]as$R){$J[$R["term"]]=array("Name"=>$R["term"],"Engine"=>"table","Rows"=>$R["count"],);if($C!=""&&$C==$R["term"])return$J[$C];}}return$J;}function
error(){global$g;return
h($g->error);}function
information_schema(){}function
is_view($S){}function
...
as$R)$J=$J&&$g->query(urlencode($R),array(),'DELETE');return$J;}$x="elastic";$Xe=array("=","query");$Zc=array();$ed=array();$gc=array(array("json"));}$Yb=array("server"=>"MySQL")+$Yb;if(!defined("DRIVER")){$Gf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
MySQLi{var$extension="MySQLi";function
__construct(){parent::init();}function
connect($N="",$V="",$G="",$k=null,$Cf=null,$Lg=null){mysqli_report(MYSQLI_REPORT_OFF);list($md,$Cf)=explode(":",$N,2);$J=@$this->real_connect(($N!=""?$md:ini_get("mysqli.default_host")),($N.$V!=""?$V:ini_get("mysqli.default_user")),($N.$V.$G!=""?$G:ini_get("mysqli.default_pw")),$k,(is_numeric($Cf)?$Cf:ini_get("mysqli.default_port")),(!is_numeric($Cf)?$Cf:$Lg));return$J;}function
...
as$y)$gi[$y]="$y = VALUES($y)";$Yg="\nON DUPLICATE KEY UPDATE ".implode(", ",$gi);$gi=array();$be=0;foreach($L
as$O){$Y="(".implode(", ",$O).")";if($gi&&(strlen($Hf)+$be+strlen($Y)+strlen($Yg)>1e6)){if(!queries($Hf.implode(",\n",$gi).$Yg))return
false;$gi=array();$be=0;}$gi[]=$Y;$be+=strlen($Y)+2;}return
queries($Hf.implode(",\n",$gi).$Yg);}}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
table($u){return
idf_escape($u);}function
connect(){global$b,$Oh,$Vg;$g=new
Min_DB;$j=$b->credentials();if($g->connect($j[0],$j[1],$j[2])){$g->set_charset(charset($g));$g->query("SET sql_quote_show_create = 1, autocommit = 1");if(version_compare($g->server_info,'5.7.8')>=0){$Vg[lang(23)][]="json";$Oh["json"]=4294967295;}return$g;}$J=$g->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($ug=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$ug;return$J;}function
...
foreign_keys($R){global$g,$Se;static$_f='`(?:[^`]|``)+`';$J=array();$Cb=$g->result("SHOW CREATE TABLE ".table($R),1);if($Cb){preg_match_all("~CONSTRAINT ($_f) FOREIGN KEY ?\\(((?:$_f,? ?)+)\\) REFERENCES ($_f)(?:\\.($_f))? \\(((?:$_f,? ?)+)\\)(?: ON DELETE ($Se))?(?: ON UPDATE ($Se))?~",$Cb,$je,PREG_SET_ORDER);foreach($je
as$B){preg_match_all("~$_f~",$B[2],$Ng);preg_match_all("~$_f~",$B[5],$lh);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Ng[0]),"target"=>array_map('idf_unescape',$lh[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($C){global$g;return
...
rename_database($C,$nb){$J=false;if(create_database($C,$nb)){$hg=array();foreach(tables_list()as$R=>$U)$hg[]=table($R)." TO ".idf_escape($C).".".table($R);$J=(!$hg||queries("RENAME TABLE ".implode(", ",$hg)));if($J)queries("DROP DATABASE ".idf_escape(DB));restart_session();set_session("dbs",null);}return$J;}function
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$li,$lh){$hg=array();foreach(array_merge($T,$li)as$R)$hg[]=table($R)." TO ".idf_escape($lh).".".table($R);return
...
routine($C,$U){global$g,$qc,$yd,$Oh;$Ca=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Nh="((".implode("|",array_merge(array_keys($Oh),$Ca)).")\\b(?:\\s*\\(((?:[^'\")]|$qc)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";$_f="\\s*(".($U=="FUNCTION"?"":$yd).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Nh";$i=$g->result("SHOW CREATE $U ".idf_escape($C),2);preg_match("~\\(((?:$_f\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$Nh\\s+":"")."(.*)~is",$i,$B);$p=array();preg_match_all("~$_f\\s*,?~is",$B[1],$je,PREG_SET_ORDER);foreach($je
as$pf){$C=str_replace("``","`",$pf[2]).$pf[3];$p[]=array("field"=>$C,"type"=>strtolower($pf[5]),"length"=>preg_replace_callback("~$qc~s",'normalize_enum',$pf[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$pf[8] $pf[7]"))),"null"=>1,"full_type"=>$pf[4],"inout"=>strtoupper($pf[1]),"collation"=>strtolower($pf[9]),);}if($U!="FUNCTION")return
...
selectLinks($ch,$O=""){echo'<p class="links">';$ee=array("select"=>lang(39));if(support("table")||support("indexes"))$ee["table"]=lang(40);if(support("table")){if(is_view($ch))$ee["view"]=lang(41);else$ee["create"]=lang(42);}if($O!==null)$ee["edit"]=lang(43);foreach($ee
...
selectQuery($H,$sh){global$x;return"<p><code class='jush-$x'>".h(str_replace("\n"," ",$H))."</code> <span class='time'>($sh)</span>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>":"")."</p>";}function
sqlCommandQuery($H){return
shorten_utf8(trim($H),1000);}function
rowDescription($R){return"";}function
rowDescriptions($L,$Sc){return$L;}function
selectLink($X,$o){}function
selectVal($X,$_,$o,$kf){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$o["type"])&&!preg_match("~var~",$o["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$o["type"])&&!is_utf8($X))$J="<i>".lang(44,strlen($kf))."</i>";if(preg_match('~json~',$o["type"]))$J="<code class='jush-js'>$J</code>";return($_?"<a href='".h($_)."'".(is_url($_)?" rel='noreferrer'":"").">$J</a>":$J);}function
...
as$C=>$v){ksort($v["columns"]);$Lf=array();foreach($v["columns"]as$y=>$X)$Lf[]="<i>".h($X)."</i>".($v["lengths"][$y]?"(".$v["lengths"][$y].")":"").($v["descs"][$y]?" DESC":"");echo"<tr title='".h($C)."'><th>$v[type]<td>".implode(", ",$Lf)."\n";}echo"</table>\n";}function
...
selectOrderPrint($cf,$e,$w){print_fieldset("sort",lang(55),$cf);$s=0;foreach((array)$_GET["order"]as$y=>$X){if($X!=""){echo"<div>".select_input(" name='order[$s]' onchange='selectFieldChange(this.form);'",$e,$X),checkbox("desc[$s]",1,isset($_GET["desc"][$y]),lang(56))."</div>\n";$s++;}}echo"<div>".select_input(" name='order[$s]' onchange='selectAddRow(this);'",$e),checkbox("desc[$s]",1,false,lang(56))."</div>\n","</div></fieldset>\n";}function
...
messageQuery($H,$sh){global$x;restart_session();$jd=&get_session("queries");$t="sql-".count($jd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$jd[$_GET["db"]][]=array($H,time(),$sh);return" <span class='time'>".@date("H:i:s")."</span> <a href='#$t' onclick=\"return !toggle('$t');\">".lang(61)."</a>"."<div id='$t' class='hidden'><pre><code class='jush-$x'>".shorten_utf8($H,1000).'</code></pre>'.($sh?" <span class='time'>($sh)</span>":'').(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($jd[$_GET["db"]])-1)).'">'.lang(10).'</a>':'').'</div>';}function
editFunctions($o){global$gc;$J=($o["null"]?"NULL/":"");foreach($gc
as$y=>$Zc){if(!$y||(!isset($_GET["call"])&&(isset($_GET["select"])||where($_GET)))){foreach($Zc
as$_f=>$X){if(!$_f||preg_match("~$_f~",$o["type"]))$J.="/$X";}if($y&&!preg_match('~set|blob|bytea|raw|file~',$o["type"]))$J.="/SQL";}}if($o["auto_increment"]&&!isset($_GET["select"])&&!where($_GET))$J=lang(48);return
explode("/",$J);}function
...
dumpTable($R,$Wg,$Hd=0){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($Wg)dump_csv(array_keys(fields($R)));}else{if($Hd==2){$p=array();foreach(fields($R)as$C=>$o)$p[]=idf_escape($C)." $o[full_type]";$i="CREATE TABLE ".table($R)." (".implode(", ",$p).")";}else$i=create_sql($R,$_POST["auto_increment"]);set_utf8mb4($i);if($Wg&&$i){if($Wg=="DROP+CREATE"||$Hd==1)echo"DROP ".($Hd==2?"VIEW":"TABLE")." IF EXISTS ".table($R).";\n";if($Hd==1)$i=remove_definer($i);echo"$i;\n\n";}}}function
dumpData($R,$Wg,$H){global$g,$x;$le=($x=="sqlite"?0:1048576);if($Wg){if($_POST["format"]=="sql"){if($Wg=="TRUNCATE+INSERT")echo
truncate_sql($R).";\n";$p=fields($R);}$I=$g->query($H,1);if($I){$_d="";$Xa="";$Od=array();$Yg="";$Hc=($R!=''?'fetch_assoc':'fetch_row');while($K=$I->$Hc()){if(!$Od){$gi=array();foreach($K
as$X){$o=$I->fetch_field();$Od[]=$o->name;$y=idf_escape($o->name);$gi[]="$y = VALUES($y)";}$Yg=($Wg=="INSERT+UPDATE"?"\nON DUPLICATE KEY UPDATE ".implode(", ",$gi):"").";\n";}if($_POST["format"]!="sql"){if($Wg=="table"){dump_csv($Od);$Wg="INSERT";}dump_csv($K);}else{if(!$_d)$_d="INSERT INTO ".table($R)." (".implode(", ",array_map('idf_escape',$Od)).") VALUES";foreach($K
as$y=>$X){$o=$p[$y];$K[$y]=($X!==null?unconvert_field($o,preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&$X!=''?$X:q($X)):"NULL");}$ug=($le?"\n":" ")."(".implode(",\t",$K).")";if(!$Xa)$Xa=$_d.$ug;elseif(strlen($Xa)+4+strlen($ug)+strlen($Yg)<$le)$Xa.=",$ug";else{echo$Xa.$Yg;$Xa=$_d.$ug;}}}if($Xa)echo$Xa.$Yg;}elseif($_POST["format"]=="sql")echo"-- ".str_replace("\n"," ",$g->error)."\n";}}function
dumpFilename($od){return
friendly_url($od!=""?$od:(SERVER!=""?SERVER:"localhost"));}function
dumpHeaders($od,$_e=false){$nf=$_POST["output"];$Bc=(preg_match('~sql~',$_POST["format"])?"sql":($_e?"tar":"csv"));header("Content-Type: ".($nf=="gz"?"application/x-gzip":($Bc=="tar"?"application/x-tar":($Bc=="sql"||$nf!="file"?"text/plain":"text/csv")."; charset=utf-8")));if($nf=="gz")ob_start('ob_gzencode',1e6);return$Bc;}function
...
true;}function
navigation($ze){global$ia,$x,$Yb,$g;echo'<h1>
...
';if($ze=="auth"){$Mc=true;foreach((array)$_SESSION["pwds"]as$ii=>$Hg){foreach($Hg
as$N=>$di){foreach($di
as$V=>$G){if($G!==null){if($Mc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Mc=false;}$Kb=$_SESSION["db"][$ii][$N][$V];foreach(($Kb?array_keys($Kb):array(""))as$m)echo"<a href='".h(auth_url($ii,$N,$V,$m))."'>($Yb[$ii]) ".h($V.($N!=""?"@$N":"").($m!=""?" - $m":""))."</a><br>\n";}}}}}else{if($_GET["ns"]!==""&&!$ze&&DB!=""){$g->select_db(DB);$T=table_status('',true);}echo'<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=jush.js&amp;version=4.3.2-dev",'"></script>
...
page_header($vh,$n="",$Wa=array(),$wh=""){global$ca,$ia,$b,$Yb,$x;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$xh=$vh.($wh!=""?": $wh":"");$yh=strip_tags($xh.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
as$y=>$X){$Pb=(is_array($X)?$X[1]:h($X));if($Pb!="")echo"<a href='".h(ME."$y=").urlencode(is_array($X)?$X[0]:$X)."'>$Pb</a> &raquo; ";}}echo"$vh\n";}}echo"<h2>$xh</h2>\n","<div id='ajaxstatus' class='jsonly hidden'></div>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$Xh=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$ve=$_SESSION["messages"][$Xh];if($ve){echo"<div class='message'>".implode("</div>\n<div class='message'>",$ve)."</div>\n";unset($_SESSION["messages"][$Xh]);}if($n)echo"<div class='error'>$n</div>\n";}function
page_footer($ze=""){global$b,$Ah;echo'</div>

';switch_lang();if($ze!="auth"){echo'<form action="" method="post">
<p class="logout">
...
long2str($W,$ni){$ug='';foreach($W
as$X)$ug.=pack('V',$X);if($ni)return
substr($ug,0,end($W));return$ug;}function
str2long($ug,$ni){$W=array_values(unpack('V*',str_pad($ug,4*ceil(strlen($ug)/4),"\0")));if($ni)$W[]=strlen($ug);return$W;}function
xxtea_mx($ti,$si,$Zg,$Kd){return
int32((($ti>>5&0x7FFFFFF)^$si<<2)+(($si>>3&0x1FFFFFFF)^$ti<<4))^int32(($Zg^$si)+($Kd^$ti));}function
encrypt_string($Ug,$y){if($Ug=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Ug,true);$Be=count($W)-1;$ti=$W[$Be];$si=$W[0];$Sf=floor(6+52/($Be+1));$Zg=0;while($Sf-->0){$Zg=int32($Zg+0x9E3779B9);$fc=$Zg>>2&3;for($of=0;$of<$Be;$of++){$si=$W[$of+1];$Ae=xxtea_mx($ti,$si,$Zg,$y[$of&3^$fc]);$ti=int32($W[$of]+$Ae);$W[$of]=$ti;}$si=$W[0];$Ae=xxtea_mx($ti,$si,$Zg,$y[$of&3^$fc]);$ti=int32($W[$Be]+$Ae);$W[$Be]=$ti;}return
long2str($W,false);}function
decrypt_string($Ug,$y){if($Ug=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Ug,false);$Be=count($W)-1;$ti=$W[$Be];$si=$W[0];$Sf=floor(6+52/($Be+1));$Zg=int32($Sf*0x9E3779B9);while($Zg){$fc=$Zg>>2&3;for($of=$Be;$of>0;$of--){$ti=$W[$of-1];$Ae=xxtea_mx($ti,$si,$Zg,$y[$of&3^$fc]);$si=int32($W[$of]-$Ae);$W[$of]=$si;}$ti=$W[$Be];$Ae=xxtea_mx($ti,$si,$Zg,$y[$of&3^$fc]);$si=int32($W[0]-$Ae);$W[0]=$si;$Zg=int32($Zg-0x9E3779B9);}return
long2str($W,true);}$g='';$id=$_SESSION["token"];if(!$id)$_SESSION["token"]=rand(1,1e6);$Ah=get_token();$Af=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$Af[$y]=$X;}}function
add_invalid_login(){global$b;$Kc=get_temp_dir()."/adminer.invalid";$Xc=@fopen($Kc,"r+");if(!$Xc){$Xc=@fopen($Kc,"w");if(!$Xc)return;}flock($Xc,LOCK_EX);$Cd=unserialize(stream_get_contents($Xc));$sh=time();if($Cd){foreach($Cd
as$Dd=>$X){if($X[0]<$sh)unset($Cd[$Dd]);}}$Bd=&$Cd[$b->bruteForceKey()];if(!$Bd)$Bd=array($sh+30*60,0);$Bd[1]++;$Fg=serialize($Cd);rewind($Xc);fwrite($Xc,$Fg);ftruncate($Xc,strlen($Fg));flock($Xc,LOCK_UN);fclose($Xc);}$Ka=$_POST["auth"];if($Ka){$Cd=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Bd=$Cd[$b->bruteForceKey()];$He=($Bd[1]>30?$Bd[0]-time():0);if($He>0)auth_error(lang(78,ceil($He/60)));session_regenerate_id();$ii=$Ka["driver"];$N=$Ka["server"];$V=$Ka["username"];$G=(string)$Ka["password"];$m=$Ka["db"];set_password($ii,$N,$V,$G);$_SESSION["db"][$ii][$N][$V][$m]=true;if($Ka["permanent"]){$y=base64_encode($ii)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($m);$Mf=$b->permanentLogin(true);$Af[$y]="$y:".base64_encode($Mf?encrypt_string($G,$Mf):"");cookie("adminer_permanent",implode(" ",$Af));}if(count($_POST)==1||DRIVER!=$ii||SERVER!=$N||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($ii,$N,$V,$m));}elseif($_POST["logout"]){if($id&&!verify_token()){page_header(lang(77),lang(79));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(80).' '.lang(81,'https://sourceforge.net/donate/index.php?group_id=264133'));}}elseif($Af&&!$_SESSION["pwds"]){session_regenerate_id();$Mf=$b->permanentLogin();foreach($Af
as$y=>$X){list(,$hb)=explode(":",$X);list($ii,$N,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($ii,$N,$V,decrypt_string(base64_decode($hb),$Mf));$_SESSION["db"][$ii][$N][$V][$m]=true;}}function
unset_permanent(){global$Af;foreach($Af
as$y=>$X){list($ii,$N,$V,$m)=array_map('base64_decode',explode("-",$y));if($ii==DRIVER&&$N==SERVER&&$V==$_GET["username"]&&$m==DB)unset($Af[$y]);}cookie("adminer_permanent",implode(" ",$Af));}function
auth_error($n){global$b,$id;$Ig=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Ig]||$_GET[$Ig])&&!$id)$n=lang(82);else{add_invalid_login();$G=get_password();if($G!==null){if($G===false)$n.='<br>'.lang(83,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Ig]&&$_GET[$Ig]&&ini_bool("session.use_only_cookies"))$n=lang(84);$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header(lang(36),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(85),lang(86,implode(", ",$Gf)),false);page_footer("auth");exit;}$g=connect();}$Xb=new
Min_Driver($g);if(!is_object($g)||($fe=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($g)?h($g):(is_string($fe)?$fe:lang(87))));if($Ka&&$_POST["token"])$_POST["token"]=$Ah;$n='';if($_POST){if(!verify_token()){$xd="max_input_vars";$pe=ini_get($xd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$pe||$X<$pe)){$xd=$y;$pe=$X;}}}$n=(!$_POST["token"]&&$pe?lang(88,"'$xd'"):lang(79).' '.lang(89));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(90,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(91);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
select($I,$h=null,$ff=array(),$z=0){global$x;$ee=array();$w=array();$e=array();$Ta=array();$Oh=array();$J=array();odd('');for($s=0;(!$z||$s<$z)&&($K=$I->fetch_row());$s++){if(!$s){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($Jd=0;$Jd<count($K);$Jd++){$o=$I->fetch_field();$C=$o->name;$ef=$o->orgtable;$df=$o->orgname;$J[$o->table]=$ef;if($ff&&$x=="sql")$ee[$Jd]=($C=="table"?"table=":($C=="possible_keys"?"indexes=":null));elseif($ef!=""){if(!isset($w[$ef])){$w[$ef]=array();foreach(indexes($ef,$h)as$v){if($v["type"]=="PRIMARY"){$w[$ef]=array_flip($v["columns"]);break;}}$e[$ef]=$w[$ef];}if(isset($e[$ef][$df])){unset($e[$ef][$df]);$w[$ef][$df]=$Jd;$ee[$Jd]=$ef;}}if($o->charsetnr==63)$Ta[$Jd]=true;$Oh[$Jd]=$o->type;echo"<th".($ef!=""||$o->name!=$df?" title='".h(($ef!=""?"$ef.":"").$df)."'":"").">".h($C).($ff?doc_link(array('sql'=>"explain-output.html#explain_".strtolower($C))):"");}echo"</thead>\n";}echo"<tr".odd().">";foreach($K
as$y=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ta[$y]&&!is_utf8($X))$X="<i>".lang(44,strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Oh[$y]==254)$X="<code>$X</code>";}if(isset($ee[$y])&&!$e[$ee[$y]]){if($ff&&$x=="sql"){$R=$K[array_search("table=",$ee)];$_=$ee[$y].urlencode($ff[$R]!=""?$ff[$R]:$R);}else{$_="edit=".urlencode($ee[$y]);foreach($w[$ee[$y]]as$lb=>$Jd)$_.="&where".urlencode("[".bracket_escape($lb)."]")."=".urlencode($K[$Jd]);}$X="<a href='".h(ME.$_)."'>$X</a>";}echo"<td>$X";}}echo($s?"</table>":"<p class='message'>".lang(12))."\n";return$J;}function
referencable_primary($Bg){$J=array();foreach(table_status('',true)as$dh=>$R){if($dh!=$Bg&&fk_support($R)){foreach(fields($dh)as$o){if($o["primary"]){if($J[$dh]){unset($J[$dh]);break;}$J[$dh]=$o;}}}}return$J;}function
...
edit_type($y,$o,$ob,$Tc=array()){global$Vg,$Oh,$Vh,$Se;$U=$o["type"];echo'<td><select name="',h($y),'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),' aria-labelledby="label-type">';if($U&&!isset($Oh[$U])&&!isset($Tc[$U]))array_unshift($Vg,$U);if($Tc)$Vg[lang(92)]=$Tc;echo
optionlist($Vg,$U),'</select>
<td><input name="',h($y),'[length]" value="',h($o["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$o["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';echo"<select name='".h($y)."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.lang(93).')'.optionlist($ob,$o["collation"]).'</select>',($Vh?"<select name='".h($y)."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($Vh,$o["unsigned"]).'</select>':''),(isset($o['on_update'])?"<select name='".h($y)."[on_update]'".(preg_match('~timestamp|datetime~',$U)?"":" class='hidden'").'>'.optionlist(array(""=>"(".lang(94).")","CURRENT_TIMESTAMP"),$o["on_update"]).'</select>':''),($Tc?"<select name='".h($y)."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".lang(95).")".optionlist(explode("|",$Se),$o["on_delete"])."</select> ":" ");}function
process_length($be){global$qc;return(preg_match("~^\\s*\\(?\\s*$qc(?:\\s*,\\s*$qc)*+\\s*\\)?\\s*\$~",$be)&&preg_match_all("~$qc~",$be,$je)?"(".implode(",",$je[0]).")":preg_replace('~^[0-9].*~','(\0)',preg_replace('~[^-0-9,+()[\]]~','',$be)));}function
process_type($o,$mb="COLLATE"){global$Vh;return" $o[type]".process_length($o["length"]).(preg_match('~(^|[^o])int|float|double|decimal~',$o["type"])&&in_array($o["unsigned"],$Vh)?" $o[unsigned]":"").(preg_match('~char|text|enum|set~',$o["type"])&&$o["collation"]?" $mb ".q($o["collation"]):"");}function
process_field($o,$Mh){global$x;$Mb=$o["default"];return
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="',lang(48),'">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default">',lang(49),(support("comment")?"<td id='label-comment'".($tb?"":" class='hidden'").">".lang(47):"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.3.2-dev' alt='+' title='".lang(100)."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$s=>$o){$s++;$gf=$o[($_POST?"orig":"field")];$Tb=(isset($_POST["add"][$s-1])||(isset($o["field"])&&!$_POST["drop_col"][$s]))&&(support("drop_col")||$gf=="");echo'<tr',($Tb?"":" style='display: none;'"),'>
',($U=="PROCEDURE"?"<td>".html_select("fields[$s][inout]",explode("|",$yd),$o["inout"]):""),'<th>';if($Tb){echo'<input name="fields[',$s,'][field]" value="',h($o["field"]),'" onchange="editingNameChange(this);',($o["field"]!=""||count($p)>1?'':' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'),'" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';}echo'<input type="hidden" name="fields[',$s,'][orig]" value="',h($gf),'">
...
drop_create($Zb,$i,$ac,$ph,$cc,$A,$ue,$se,$te,$Oe,$Ee){if($_POST["drop"])query_redirect($Zb,$A,$ue);elseif($Oe=="")query_redirect($i,$A,$te);elseif($Oe!=$Ee){$Db=queries($i);queries_redirect($A,$se,$Db&&queries($Zb));if($Db)queries($ac);}else
queries_redirect($A,$se,queries($ph)&&queries($cc)&&queries($Zb)&&queries($i));}function
create_trigger($Re,$K){global$x;$uh=" $K[Timing] $K[Event]".($K["Event"]=="UPDATE OF"?" ".idf_escape($K["Of"]):"");return"CREATE TRIGGER ".idf_escape($K["Trigger"]).($x=="mssql"?$Re.$uh:$uh.$Re).rtrim(" $K[Type]\n$K[Statement]",";").";";}function
create_routine($qg,$K){global$yd;$O=array();$p=(array)$K["fields"];ksort($p);foreach($p
as$o){if($o["field"]!="")$O[]=(preg_match("~^($yd)\$~",$o["inout"])?"$o[inout] ":"").idf_escape($o["field"]).process_type($o,"CHARACTER SET");}return"CREATE $qg ".idf_escape(trim($K["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($K["returns"],"CHARACTER SET"):"").($K["language"]?" LANGUAGE $K[language]":"").rtrim("\n$K[definition]",";").";";}function
remove_definer($H){return
preg_replace('~^([A-Z =]+) DEFINER=`'.preg_replace('~@(.*)~','`@`(%|\\1)',logged_user()).'`~','\\1',$H);}function
format_foreign_key($q){global$Se;return" FOREIGN KEY (".implode(", ",array_map('idf_escape',$q["source"])).") REFERENCES ".table($q["table"])." (".implode(", ",array_map('idf_escape',$q["target"])).")".(preg_match("~^($Se)\$~",$q["on_delete"])?" ON DELETE $q[on_delete]":"").(preg_match("~^($Se)\$~",$q["on_update"])?" ON UPDATE $q[on_update]":"");}function
tar_file($Kc,$zh){$J=pack("a100a8a8a8a12a12",$Kc,644,0,0,decoct($zh->size),decoct(time()));$fb=8*32;for($s=0;$s<strlen($J);$s++)$fb+=ord($J[$s]);$J.=sprintf("%06o",$fb)."\0 ";echo$J,str_repeat("\0",512-strlen($J));$zh->send();echo
...
connect_error(){global$b,$g,$Ah,$n,$Yb;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header(lang(35).": ".h(DB),lang(104),true);}else{if($_POST["db"]&&!$n)queries_redirect(substr(ME,0,-1),lang(105),drop_databases($_POST["db"]));page_header(lang(106),$n,false);echo"<p class='links'>\n";foreach(array('database'=>lang(107),'privileges'=>lang(68),'processlist'=>lang(108),'variables'=>lang(109),'status'=>lang(110),)as$y=>$X){if(support($y))echo"<a href='".h(ME)."$y='>$X</a>\n";}echo"<p>".lang(111,$Yb[DRIVER],"<b>".h($g->server_info)."</b>","<b>$g->extension</b>")."\n","<p>".lang(112,"<b>".h(logged_user())."</b>")."\n";$l=$b->databases();if($l){$xg=support("scheme");$ob=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".lang(35)." - <a href='".h(ME)."refresh=1'>".lang(113)."</a>"."<td>".lang(114)."<td>".lang(115)."<td>".lang(116)." - <a href='".h(ME)."dbsize=1' onclick=\"return !ajaxSetHtml('".h(js_escape(ME))."script=connect');\">".lang(117)."</a>"."</thead>\n";$l=($_GET["dbsize"]?count_tables($l):array_flip($l));foreach($l
as$m=>$T){$pg=h(ME)."db=".urlencode($m);$t=h("Db-".$m);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$m,in_array($m,(array)$_POST["db"]),"","","",$t):""),"<th><a href='$pg' id='$m'>".h($m)."</a>";$nb=nbsp(db_collation($m,$ob));echo"<td>".(support("database")?"<a href='$pg".($xg?"&amp;ns=":"")."&amp;database=' title='".lang(64)."'>$nb</a>":$nb),"<td align='right'><a href='$pg&amp;schema=' id='tables-".h($m)."' title='".lang(67)."'>".($_GET["dbsize"]?$T:"?")."</a>","<td align='right' id='size-".h($m)."'>".($_GET["dbsize"]?db_size($m):"?"),"\n";}echo"</table>\n",(support("database")?"<fieldset><legend>".lang(118)." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^db/));\">\n"."<input type='submit' name='drop' value='".lang(119)."'".confirm().">\n"."</div></fieldset>\n":""),"<script type='text/javascript'>tableCheck();</script>\n","<input type='hidden' name='token' value='$Ah'>\n","</form>\n";}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header(lang(73).": ".h($_GET["ns"]),lang(120),true);page_footer("ns");exit;}}$Se="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
...
as$y=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($y)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($y))."'>".lang(128)."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.lang(131)."</a>\n";}}elseif(isset($_GET["schema"])){page_header(lang(67),"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$fh=array();$gh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$je,PREG_SET_ORDER);foreach($je
as$s=>$B){$fh[$B[1]]=array($B[2],$B[3]);$gh[]="\n\t'".js_escape($B[1])."': [ $B[2], $B[3] ]";}$Bh=0;$Qa=-1;$wg=array();$dg=array();$Zd=array();foreach(table_status('',true)as$R=>$S){if(is_view($S))continue;$Df=0;$wg[$R]["fields"]=array();foreach(fields($R)as$C=>$o){$Df+=1.25;$o["pos"]=$Df;$wg[$R]["fields"][$C]=$o;}$wg[$R]["pos"]=($fh[$R]?$fh[$R]:array($Bh,0));foreach($b->foreignKeys($R)as$X){if(!$X["db"]){$Xd=$Qa;if($fh[$R][1]||$fh[$X["table"]][1])$Xd=min(floatval($fh[$R][1]),floatval($fh[$X["table"]][1]))-1;else$Qa-=.1;while($Zd[(string)$Xd])$Xd-=.0001;$wg[$R]["references"][$X["table"]][(string)$Xd]=array($X["source"],$X["target"]);$dg[$X["table"]][$R][(string)$Xd]=$X["target"];$Zd[(string)$Xd]=true;}}$Bh=max($Bh,$wg[$R]["pos"][0]+2.5+$Df);}echo'<div id="schema" style="height: ',$Bh,'em;" onselectstart="return false;">
<script type="text/javascript">
...
';foreach($wg
as$C=>$R){echo"<div class='table' style='top: ".$R["pos"][0]."em; left: ".$R["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($C).'"><b>'.h($C)."</b></a>";foreach($R["fields"]as$o){$X='<span'.type_class($o["type"]).' title="'.h($o["full_type"].($o["null"]?" NULL":'')).'">'.h($o["field"]).'</span>';echo"<br>".($o["primary"]?"<i>$X</i>":$X);}foreach((array)$R["references"]as$mh=>$eg){foreach($eg
as$Xd=>$ag){$Yd=$Xd-$fh[$C][1];$s=0;foreach($ag[0]as$Ng)echo"\n<div class='references' title='".h($mh)."' id='refs$Xd-".($s++)."' style='left: $Yd"."em; top: ".$R["fields"][$Ng]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$Yd)."em;'></div></div>";}}foreach((array)$dg[$C]as$mh=>$eg){foreach($eg
as$Xd=>$e){$Yd=$Xd-$fh[$C][1];$s=0;foreach($e
as$lh)echo"\n<div class='references' title='".h($mh)."' id='refd$Xd-".($s++)."' style='left: $Yd"."em; top: ".$R["fields"][$lh]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=4.3.2-dev'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Yd)."em;'></div></div>";}}echo"\n</div>\n";}foreach($wg
as$C=>$R){foreach((array)$R["references"]as$mh=>$eg){foreach($eg
as$Xd=>$ag){$ye=$Bh;$ne=-10;foreach($ag[0]as$y=>$Ng){$Ef=$R["pos"][0]+$R["fields"][$Ng]["pos"];$Ff=$wg[$mh]["pos"][0]+$wg[$mh]["fields"][$ag[1][$y]]["pos"];$ye=min($ye,$Ef,$Ff);$ne=max($ne,$Ef,$Ff);}echo"<div class='references' id='refl$Xd' style='left: $Xd"."em; top: $ye"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($ne-$ye)."em;'></div></div>\n";}}}echo'</div>
...
as$m){$b->dumpDatabase($m);if($g->select_db($m)){if($Fd&&preg_match('~CREATE~',$Wg)&&($i=$g->result("SHOW CREATE DATABASE ".idf_escape($m),1))){set_utf8mb4($i);if($Wg=="DROP+CREATE")echo"DROP DATABASE IF EXISTS ".idf_escape($m).";\n";echo"$i;\n";}if($Fd){if($Wg)echo
use_sql($m).";\n\n";$mf="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$qg){foreach(get_rows("SHOW $qg STATUS WHERE Db = ".q($m),null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE $qg ".idf_escape($K["Name"]),2));set_utf8mb4($i);$mf.=($Wg!='DROP+CREATE'?"DROP $qg IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$K){$i=remove_definer($g->result("SHOW CREATE EVENT ".idf_escape($K["Name"]),3));set_utf8mb4($i);$mf.=($Wg!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($K["Name"]).";;\n":"")."$i;;\n\n";}}if($mf)echo"DELIMITER ;;\n\n$mf"."DELIMITER ;\n\n";}if($_POST["table_style"]||$_POST["data_style"]){$li=array();foreach(table_status('',true)as$C=>$S){$R=(DB==""||in_array($C,(array)$_POST["tables"]));$Gb=(DB==""||in_array($C,(array)$_POST["data"]));if($R||$Gb){if($Bc=="tar"){$zh=new
TmpFile;ob_start(array($zh,'write'),1e5);}$b->dumpTable($C,($R?$_POST["table_style"]:""),(is_view($S)?2:0));if(is_view($S))$li[]=$C;elseif($Gb){$p=fields($C);$b->dumpData($C,$_POST["data_style"],"SELECT *".convert_fields($p,$p)." FROM ".table($C));}if($Fd&&$_POST["triggers"]&&$R&&($Lh=trigger_sql($C,$_POST["table_style"])))echo"\nDELIMITER ;;\n$Lh\nDELIMITER ;\n";if($Bc=="tar"){ob_end_flush();tar_file((DB!=""?"":"$m/")."$C.csv",$zh);}elseif($Fd)echo"\n";}}foreach($li
as$ki)$b->dumpTable($ki,$_POST["table_style"],1);if($Bc=="tar")echo
...
<table cellspacing="0">
';$Jb=array('','USE','DROP+CREATE','CREATE');$hh=array('','DROP+CREATE','CREATE');$Hb=array('','TRUNCATE+INSERT','INSERT');if($x=="sql")$Hb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".lang(133)."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".lang(134)."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($x=="sqlite"?"":"<tr><th>".lang(35)."<td>".html_select('db_style',$Jb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],lang(135)):"").(support("event")?checkbox("events",1,$K["events"],lang(136)):"")),"<tr><th>".lang(115)."<td>".html_select('table_style',$hh,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],lang(48)).(support("trigger")?checkbox("triggers",1,$K["triggers"],lang(130)):""),"<tr><th>".lang(137)."<td>".html_select('data_style',$Hb,$K["data_style"]),'</table>
<p><input type="submit" value="',lang(70),'">
<input type="hidden" name="token" value="',$Ah,'">

<table cellspacing="0">
';$If=array();if(DB!=""){$db=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$db onclick='formCheck(this, /^tables\\[/);'>".lang(115)."</label>","<th style='text-align: right;'><label class='block'>".lang(137)."<input type='checkbox' id='check-data'$db onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$li="";$ih=tables_list();foreach($ih
as$C=>$U){$Hf=preg_replace('~_.*~','',$C);$db=($a==""||$a==(substr($a,-1)=="%"?"$Hf%":$C));$Lf="<tr><td>".checkbox("tables[]",$C,$db,$C,"checkboxClick(event, this); formUncheck('check-tables');","block");if($U!==null&&!preg_match('~table~i',$U))$li.="$Lf\n";else
echo"$Lf<td align='right'><label class='block'><span id='Rows-".h($C)."'></span>".checkbox("data[]",$C,$db,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$If[$Hf]++;}echo$li;if($ih)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}else{echo"<thead><tr><th style='text-align: left;'><label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".lang(35)."</label></thead>\n";$l=$b->databases();if($l){foreach($l
as$m){if(!information_schema($m)){$Hf=preg_replace('~_.*~','',$m);echo"<tr><td>".checkbox("databases[]",$m,$a==""||$a=="$Hf%",$m,"formUncheck('check-databases');","block")."\n";$If[$Hf]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$Mc=true;foreach($If
as$y=>$X){if($y!=""&&$X>1){echo($Mc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$y%")."'>".h($y)."</a>";$Mc=false;}}}elseif(isset($_GET["privileges"])){page_header(lang(68));echo'<p class="links"><a href="'.h(ME).'user=">'.lang(138)."</a>";$I=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ad=$I;if(!$I)$I=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ad?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".lang(33)."<th>".lang(32)."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.lang(10)."</a>\n";if(!$ad||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".lang(10)."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$kd=&get_session("queries");$jd=&$kd[DB];if(!$n&&$_POST["clear"]){$jd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?lang(69):lang(61)),$n);if(!$n&&$_POST){$Xc=false;if(!isset($_GET["import"]))$H=$_POST["query"];elseif($_POST["webfile"]){$Xc=@fopen((file_exists("adminer.sql")?"adminer.sql":"compress.zlib://adminer.sql.gz"),"rb");$H=($Xc?fread($Xc,1e6):false);}else$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$Sf=$H.(preg_match("~;[ \t\r\n]*\$~",$H)?"":";");if(!$jd||reset(end($jd))!=$Sf){restart_session();$jd[]=array($Sf,time());set_session("queries",$kd);stop_session();}}$Og="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$Ob=";";$D=0;$nc=true;$h=connect();if(is_object($h)&&DB!="")$h->select_db(DB);$rb=0;$sc=array();$rf='[\'"'.($x=="sql"?'`#':($x=="sqlite"?'`[':($x=="mssql"?'[':''))).']|/\\*|-- |$'.($x=="pgsql"?'|\\$[^$]*\\$':'');$Ch=microtime(true);parse_str($_COOKIE["adminer_export"],$xa);$ec=$b->dumpFormat();unset($ec["sql"]);while($H!=""){if(!$D&&preg_match("~^$Og*+DELIMITER\\s+(\\S+)~i",$H,$B)){$Ob=$B[1];$H=substr($H,strlen($B[0]));}else{preg_match('('.preg_quote($Ob)."\\s*|$rf)",$H,$B,PREG_OFFSET_CAPTURE,$D);list($Vc,$Df)=$B[0];if(!$Vc&&$Xc&&!feof($Xc))$H.=fread($Xc,1e5);else{if(!$Vc&&rtrim($H)=="")break;$D=$Df+strlen($Vc);if($Vc&&rtrim($Vc)!=$Ob){while(preg_match('('.($Vc=='/*'?'\\*/':($Vc=='['?']':(preg_match('~^-- |^#~',$Vc)?"\n":preg_quote($Vc)."|\\\\."))).'|$)s',$H,$B,PREG_OFFSET_CAPTURE,$D)){$ug=$B[0][0];if(!$ug&&$Xc&&!feof($Xc))$H.=fread($Xc,1e5);else{$D=$B[0][1]+strlen($ug);if($ug[0]!="\\")break;}}}else{$nc=false;$Sf=substr($H,0,$Df);$rb++;$Lf="<pre id='sql-$rb'><code class='jush-$x'>".$b->sqlCommandQuery($Sf)."</code></pre>\n";if($x=="sqlite"&&preg_match("~^$Og*+ATTACH\\b~i",$Sf,$B)){echo$Lf,"<p class='error'>".lang(139)."\n";$sc[]=" <a href='#sql-$rb'>$rb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$Lf;ob_flush();flush();}$Sg=microtime(true);if($g->multi_query($Sf)&&is_object($h)&&preg_match("~^$Og*+USE\\b~i",$Sf))$h->query($Sf);do{$I=$g->store_result();$sh=" <span class='time'>(".format_time($Sg).")</span>".(strlen($Sf)<1000?" <a href='".h(ME)."sql=".urlencode(trim($Sf))."'>".lang(10)."</a>":"");if($g->error){echo($_POST["only_errors"]?$Lf:""),"<p class='error'>".lang(140).($g->errno?" ($g->errno)":"").": ".error()."\n";$sc[]=" <a href='#sql-$rb'>$rb</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$z=$_POST["limit"];$ff=select($I,$h,array(),$z);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$Ie=$I->num_rows;echo"<p>".($Ie?($z&&$Ie>$z?lang(141,$z):"").lang(142,$Ie):""),$sh;$t="export-$rb";$Ac=", <a href='#$t' onclick=\"return !toggle('$t');\">".lang(70)."</a><span id='$t' class='hidden'>: ".html_select("output",$b->dumpOutput(),$xa["output"])." ".html_select("format",$ec,$xa["format"])."<input type='hidden' name='query' value='".h($Sf)."'>"." <input type='submit' name='export' value='".lang(70)."'><input type='hidden' name='token' value='$Ah'></span>\n";if($h&&preg_match("~^($Og|\\()*+SELECT\\b~i",$Sf)&&($_c=explain($h,$Sf))){$t="explain-$rb";echo", <a href='#$t' onclick=\"return !toggle('$t');\">EXPLAIN</a>$Ac","<div id='$t' class='hidden'>\n";select($_c,$h,$ff);echo"</div>\n";}else
echo$Ac;echo"</form>\n";}}else{if(preg_match("~^$Og*+(CREATE|DROP|ALTER)$Og++(DATABASE|SCHEMA)\\b~i",$Sf)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($g->info)."'>".lang(143,$g->affected_rows)."$sh\n";}$Sg=microtime(true);}while($g->next_result());}$H=substr($H,$D);$D=0;}}}}if($nc)echo"<p class='message'>".lang(144)."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".lang(145,$rb-count($sc))," <span class='time'>(".format_time($Ch).")</span>\n";}elseif($sc&&$rb>1)echo"<p class='error'>".lang(140).": ".implode("",$sc)."\n";}else
echo"<p class='error'>".upload_error($H)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$xc="<input type='submit' value='".lang(146)."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$Sf=$_GET["sql"];if($_POST)$Sf=$_POST["query"];elseif($_GET["history"]=="all")$Sf=$jd;elseif($_GET["history"]!="")$Sf=$jd[$_GET["history"]][0];echo"<p>";textarea("query",$Sf,20);echo($_POST?"":"<script type='text/javascript'>document.getElementsByTagName('textarea')[0].focus();</script>\n"),"<p>$xc\n",lang(147).": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".lang(148)."</legend><div>",(ini_bool("file_uploads")?"SQL (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$xc":lang(149)),"</div></fieldset>\n","<fieldset><legend>".lang(150)."</legend><div>",lang(151,"<code>adminer.sql".(extension_loaded("zlib")?"[.gz]":"")."</code>"),' <input type="submit" name="webfile" value="'.lang(152).'">',"</div></fieldset>\n","<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])),lang(153))."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])),lang(154))."\n","<input type='hidden' name='token' value='$Ah'>\n";if(!isset($_GET["import"])&&$jd){print_fieldset("history",lang(155),$_GET["history"]!="");for($X=end($jd);$X;$X=prev($jd)){$y=key($jd);list($Sf,$sh,$ic)=$X;echo'<a href="'.h(ME."sql=&history=$y").'">'.lang(10)."</a>"." <span class='time' title='".@date('Y-m-d',$sh)."'>".@date("H:i:s",$sh)."</span>"." <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$Sf)))),80,"</code>").($ic?" <span class='time'>($ic)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".lang(156)."'>\n","<a href='".h(ME."sql=&history=all")."'>".lang(157)."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$C=>$o){if(!isset($o["privileges"][$Wh?"update":"insert"])||$b->fieldName($o)=="")unset($p[$C]);}if($_POST&&!$n&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Wh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$w=indexes($a);$Rh=unique_array($_GET["where"],$w);$Vf="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,lang(158),$Xb->delete($a,$Vf,!$Rh));else{$O=array();foreach($p
as$C=>$o){$X=process_input($o);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($Wh){if(!$O)redirect($A);queries_redirect($A,lang(159),$Xb->update($a,$O,$Vf,!$Rh));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$I=$Xb->insert($a,$O);$Wd=($I?last_id():0);queries_redirect($A,lang(160,($Wd?" $Wd":"")),$I);}}}$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($p
...
as$y=>$X){if(!$Z)$K[$y]=null;$p[$y]=array("field"=>$y,"null"=>($y!=$Xb->primary),"auto_increment"=>($y==$Xb->primary));}}}edit_form($a,$p,$K,$Wh);}elseif(isset($_GET["create"])){$a=$_GET["create"];$tf=array();foreach(array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST')as$y)$tf[$y]=$y;$cg=referencable_primary($a);$Tc=array();foreach($cg
as$dh=>$o)$Tc[str_replace("`","``",$dh)."`".str_replace("`","``",$o["field"])]=$dh;$if=array();$S=array();if($a!=""){$if=fields($a);$S=table_status($a);if(!$S)$n=lang(9);}$K=$_POST;$K["fields"]=(array)$K["fields"];if($K["auto_increment_col"])$K["fields"][$K["auto_increment_col"]]["auto_increment"]=true;if($_POST&&!process_fields($K["fields"])&&!$n){if($_POST["drop"])queries_redirect(substr(ME,0,-1),lang(161),drop_tables(array($a)));else{$p=array();$Da=array();$ai=false;$Rc=array();$hf=reset($if);$Aa=" FIRST";foreach($K["fields"]as$y=>$o){$q=$Tc[$o["type"]];$Mh=($q!==null?$cg[$q]:$o);if($o["field"]!=""){if(!$o["has_default"])$o["default"]=null;if($y==$K["auto_increment_col"])$o["auto_increment"]=true;$Qf=process_field($o,$Mh);$Da[]=array($o["orig"],$Qf,$Aa);if($Qf!=process_field($hf,$hf)){$p[]=array($o["orig"],$Qf,$Aa);if($o["orig"]!=""||$Aa)$ai=true;}if($q!==null)$Rc[idf_escape($o["field"])]=($a!=""&&$x!="sqlite"?"ADD":" ").format_foreign_key(array('table'=>$Tc[$o["type"]],'source'=>array($o["field"]),'target'=>array($Mh["field"]),'on_delete'=>$o["on_delete"],));$Aa=" AFTER ".idf_escape($o["field"]);}elseif($o["orig"]!=""){$ai=true;$p[]=array($o["orig"]);}if($o["orig"]!=""){$hf=next($if);if(!$hf)$Aa="";}}$vf="";if($tf[$K["partition_by"]]){$wf=array();if($K["partition_by"]=='RANGE'||$K["partition_by"]=='LIST'){foreach(array_filter($K["partition_names"])as$y=>$X){$Y=$K["partition_values"][$y];$wf[]="\n  PARTITION ".idf_escape($X)." VALUES ".($K["partition_by"]=='RANGE'?"LESS THAN":"IN").($Y!=""?" ($Y)":" MAXVALUE");}}$vf.="\nPARTITION BY $K[partition_by]($K[partition])".($wf?" (".implode(",",$wf)."\n)":($K["partitions"]?" PARTITIONS ".(+$K["partitions"]):""));}elseif(support("partitioning")&&preg_match("~partitioned~",$S["Create_options"]))$vf.="\nREMOVE PARTITIONING";$re=lang(162);if($a==""){cookie("adminer_engine",$K["Engine"]);$re=lang(163);}$C=trim($K["name"]);queries_redirect(ME.(support("table")?"table=":"select=").urlencode($C),$re,alter_table($a,$C,($x=="sqlite"&&($ai||$Rc)?$Da:$p),$Rc,($K["Comment"]!=$S["Comment"]?$K["Comment"]:null),($K["Engine"]&&$K["Engine"]!=$S["Engine"]?$K["Engine"]:""),($K["Collation"]&&$K["Collation"]!=$S["Collation"]?$K["Collation"]:""),($K["Auto_increment"]!=""?number($K["Auto_increment"]):""),$vf));}}page_header(($a!=""?lang(42):lang(71)),$n,array("table"=>$a),h($a));if(!$_POST){$K=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Oh["int"])?"int":(isset($Oh["integer"])?"integer":"")))),"partition_names"=>array(""),);if($a!=""){$K=$S;$K["name"]=$a;$K["fields"]=array();if(!$_GET["auto_increment"])$K["Auto_increment"]="";foreach($if
as$o){$o["has_default"]=isset($o["default"]);$K["fields"][]=$o;}if(support("partitioning")){$Yc="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($a);$I=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Yc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($K["partition_by"],$K["partitions"],$K["partition"])=$I->fetch_row();$wf=get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Yc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");$wf[""]="";$K["partition_names"]=array_keys($wf);$K["partition_values"]=array_values($wf);}}}$ob=collations();$pc=engines();foreach($pc
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($pc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".lang(165).")")+$pc,$K["Engine"])."</select>":""),' ',($ob&&!preg_match("~sqlite|mssql~",$x)?html_select("Collation",array(""=>"(".lang(93).")")+$ob,$K["Collation"]):""),' <input type="submit" value="',lang(14),'">
';}echo'
...
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}if(support("partitioning")){$uf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",lang(167),$K["partition_by"]);echo'<p>
',"<select name='partition_by' onchange='partitionByChange(this);'".on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')",1).">".optionlist(array(""=>"")+$tf,$K["partition_by"])."</select>",'(<input name="partition" value="',h($K["partition"]),'">)
',lang(168),': <input type="number" name="partitions" class="size',($uf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
';if($Jf){echo"<tr><td>PRIMARY<td>";foreach($Jf["columns"]as$y=>$d){echo
select_input(" disabled",$p,$d),"<label><input disabled type='checkbox'>".lang(56)."</label> ";}echo"<td><td>\n";}$Jd=1;foreach($K["indexes"]as$v){if(!$_POST["drop_col"]||$Jd!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$Jd][type]",array(-1=>"")+$td,$v["type"],($Jd==count($K["indexes"])?"indexesAddRow(this);":1),"label-type"),"<td>";ksort($v["columns"]);$s=1;foreach($v["columns"]as$y=>$d){echo"<span>".select_input(" name='indexes[$Jd][columns][$s]' onchange=\"".($s==count($v["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".h(js_escape($x=="sql"?"":$_GET["indexes"]."_"))."');\" title='".lang(45)."'",($p?array_combine($p,$p):$p),$d),($x=="sql"||$x=="mssql"?"<input type='number' name='indexes[$Jd][lengths][$s]' class='size' value='".h($v["lengths"][$y])."' title='".lang(98)."'>":""),($x!="sql"?checkbox("indexes[$Jd][descs][$s]",1,$v["descs"][$y],lang(56)):"")," </span>";$s++;}echo"<td><input name='indexes[$Jd][name]' value='".h($v["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$Jd]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.3.2-dev' alt='x' title='".lang(103)."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$Jd++;}echo'</table>
<p>
<input type="submit" value="',lang(14),'">
<input type="hidden" name="token" value="',$Ah,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$n&&!isset($_POST["add_x"])){$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),lang(175),drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),lang(176),rename_database($C,$K["collation"]));}else{$l=explode("\n",str_replace("\r","",$C));$Xg=true;$Vd="";foreach($l
as$m){if(count($l)==1||$m!=""){if(!create_database($m,$K["collation"]))$Xg=false;$Vd=$m;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($Vd),lang(177),$Xg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),lang(178));}}page_header(DB!=""?lang(64):lang(107),$n,array(),h(DB));$ob=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ob);elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$ad){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ad,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($ob?html_select("collation",array(""=>"(".lang(93).")")+$ob,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header(lang(182).": ".h($da),$n);$qg=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$rd=array();$mf=array();foreach($qg["fields"]as$s=>$o){if(substr($o["inout"],-3)=="OUT")$mf[$s]="@".idf_escape($o["field"])." AS ".idf_escape($o["field"]);if(!$o["inout"]||substr($o["inout"],0,2)=="IN")$rd[]=$s;}if(!$n&&$_POST){$Ya=array();foreach($qg["fields"]as$y=>$o){if(in_array($y,$rd)){$X=process_input($o);if($X===false)$X="''";if(isset($mf[$y]))$g->query("SET @".idf_escape($o["field"])." = $X");}$Ya[]=(isset($mf[$y])?"@".idf_escape($o["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Ya).")";echo"<p><code class='jush-$x'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".lang(10)."</a>\n";if(!$g->multi_query($H))echo"<p class='error'>".error()."\n";else{$h=connect();if(is_object($h))$h->select_db(DB);do{$I=$g->store_result();if(is_object($I))select($I,$h);else
...
as$y){$o=$qg["fields"][$y];$C=$o["field"];echo"<tr><th>".$b->fieldName($o);$Y=$_POST["fields"][$C];if($Y!=""){if($o["type"]=="enum")$Y=+$Y;if($o["type"]=="set")$Y=array_sum($Y);}input($o,$Y,(string)$_POST["function"][$C]);echo"\n";}echo"</table>\n";}echo'<p>
<input type="submit" value="',lang(182),'">
<input type="hidden" name="token" value="',$Ah,'">
</form>
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$K=$_POST;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$re=($_POST["drop"]?lang(184):($C!=""?lang(185):lang(186)));$A=ME."table=".urlencode($a);if(!$_POST["drop"]){$K["source"]=array_filter($K["source"],'strlen');ksort($K["source"]);$lh=array();foreach($K["source"]as$y=>$X)$lh[$y]=$K["target"][$y];$K["target"]=$lh;}if($x=="sqlite")queries_redirect($A,$re,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($K)))));else{$c="ALTER TABLE ".table($a);$Zb="\nDROP ".($x=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$Zb,$A,$re);else{query_redirect($c.($C!=""?"$Zb,":"")."\nADD".format_foreign_key($K),$A,$re);$n=lang(187)."<br>$n";}}}page_header(lang(188),$n,array("table"=>$a),h($a));if($_POST){ksort($K["source"]);if($_POST["add"])$K["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$K["target"]=array();}elseif($C!=""){$Tc=foreign_keys($a);$K=$Tc[$C];$K["source"][]="";}else{$K["table"]=$a;$K["source"]=array("");}$Ng=array_keys(fields($a));$lh=($a===$K["table"]?$Ng:array_keys(fields($K["table"])));$bg=array_keys(array_filter(table_status('',true),'fk_support'));echo'
<form action="" method="post">
<p>
';if($K["db"]==""&&$K["ns"]==""){echo
lang(189),':
...
';$Jd=0;foreach($K["source"]as$y=>$X){echo"<tr>","<td>".html_select("source[".(+$y)."]",array(-1=>"")+$Ng,$X,($Jd==count($K["source"])-1?"foreignAddRow(this);":1),"label-source"),"<td>".html_select("target[".(+$y)."]",$lh,$K["target"][$y],1,"label-target");$Jd++;}echo'</table>
<p>
',lang(95),': ',html_select("on_delete",array(-1=>"")+explode("|",$Se),$K["on_delete"]),' ',lang(94),': ',html_select("on_update",array(-1=>"")+explode("|",$Se),$K["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="',lang(14),'">
<noscript><p><input type="submit" name="add" value="',lang(191),'"></noscript>
';}if($C!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Ah,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;$jf="VIEW";if($x=="pgsql"&&$a!=""){$P=table_status($a);$jf=strtoupper($P["Engine"]);}if($_POST&&!$n){$C=trim($K["name"]);$Ga=" AS\n$K[select]";$A=ME."table=".urlencode($C);$re=lang(192);$U=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$x!="sqlite"&&$U=="VIEW"&&$jf=="VIEW")query_redirect(($x=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$re);else{$nh=$C."_adminer_".uniqid();drop_create("DROP $jf ".table($a),"CREATE $U ".table($C).$Ga,"DROP $U ".table($C),"CREATE $U ".table($nh).$Ga,"DROP $U ".table($nh),($_POST["drop"]?substr(ME,0,-1):$A),lang(193),$re,lang(194),$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;$K["materialized"]=($jf!="VIEW");if(!$n)$n=error();}page_header(($a!=""?lang(41):lang(195)),$n,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>',lang(203),'<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ad,$K["INTERVAL_FIELD"]),'<tr><th>',lang(110),'<td>',html_select("STATUS",$Tg,$K["STATUS"]),'<tr><th>',lang(47),'<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",lang(204)),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="',lang(14),'">
';if($aa!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Ah,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$qg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$n){$nh="$K[name]_adminer_".uniqid();drop_create("DROP $qg ".idf_escape($da),create_routine($qg,$K),"DROP $qg ".idf_escape($K["name"]),create_routine($qg,array("name"=>$nh)+$K),"DROP $qg ".idf_escape($nh),substr(ME,0,-1),lang(205),lang(206),lang(207),$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?lang(208):lang(209)).": ".h($da):(isset($_GET["function"])?lang(210):lang(211))),$n);if(!$_POST&&$da!=""){$K=routine($da,$qg);$K["name"]=$da;}$ob=get_vals("SHOW CHARACTER SET");sort($ob);$rg=routine_languages();echo'
<form action="" method="post" id="form">
<p>',lang(174),': <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($rg?lang(19).": ".html_select("language",$rg,$K["language"]):""),'<input type="submit" value="',lang(14),'">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$ob,$qg);if(isset($_GET["function"])){echo"<tr><td>".lang(212);edit_type("returns",$K["returns"],$ob);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="',lang(14),'">
';if($da!=""){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Ah,'">
</form>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Kh=trigger_options();$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$n&&in_array($_POST["Timing"],$Kh["Timing"])&&in_array($_POST["Event"],$Kh["Event"])&&in_array($_POST["Type"],$Kh["Type"])){$Re=" ON ".table($a);$Zb="DROP TRIGGER ".idf_escape($C).($x=="pgsql"?$Re:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Zb,$A,lang(222));else{if($C!="")queries($Zb);queries_redirect($A,($C!=""?lang(223):lang(224)),queries(create_trigger($Re,$_POST)));if($C!="")queries(create_trigger($Re,$K+array("Type"=>reset($Kh["Type"]))));}}$K=$_POST;}page_header(($C!=""?lang(225).": ".h($C):lang(226)),$n,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>',lang(227),'<td>',html_select("Timing",$Kh["Timing"],$K["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>',lang(228),'<td>',html_select("Event",$Kh["Event"],$K["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Kh["Event"])?" <input name='Of' value='".h($K["Of"])."' class='hidden'>":""),'<tr><th>',lang(46),'<td>',html_select("Type",$Kh["Type"],$K["Type"]),'</table>
...
';}elseif(isset($_GET["user"])){$ha=$_GET["user"];$Of=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$K){foreach(explode(",",($K["Privilege"]=="Grant option"?"":$K["Context"]))as$zb)$Of[$zb][$K["Privilege"]]=$K["Comment"];}$Of["Server Admin"]+=$Of["File access on server"];$Of["Databases"]["Create routine"]=$Of["Procedures"]["Create routine"];unset($Of["Procedures"]["Create routine"]);$Of["Columns"]=array();foreach(array("Select","Insert","Update","References")as$X)$Of["Columns"][$X]=$Of["Tables"][$X];unset($Of["Server Admin"]["Usage"]);foreach($Of["Tables"]as$y=>$X)unset($Of["Databases"][$y]);$De=array();if($_POST){foreach($_POST["objects"]as$y=>$X)$De[$X]=(array)$De[$X]+(array)$_POST["grants"][$y];}$bd=array();$Pe="";if(isset($_GET["host"])&&($I=$g->query("SHOW GRANTS FOR ".q($ha)."@".q($_GET["host"])))){while($K=$I->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$K[0],$B)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$B[1],$je,PREG_SET_ORDER)){foreach($je
as$X){if($X[1]!="USAGE")$bd["$B[2]$X[2]"][$X[1]]=true;if(preg_match('~ WITH GRANT OPTION~',$K[0]))$bd["$B[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$B))$Pe=$B[1];}}if($_POST&&!$n){$Qe=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Qe",ME."privileges=",lang(229));else{$Fe=q($_POST["user"])."@".q($_POST["host"]);$xf=$_POST["pass"];if($xf!=''&&!$_POST["hashed"]){$xf=$g->result("SELECT PASSWORD(".q($xf).")");$n=!$xf;}$Db=false;if(!$n){if($Qe!=$Fe){$Db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Fe IDENTIFIED BY PASSWORD ".q($xf));$n=!$Db;}elseif($xf!=$Pe)queries("SET PASSWORD FOR $Fe = ".q($xf));}if(!$n){$ng=array();foreach($De
as$Ke=>$ad){if(isset($_GET["grant"]))$ad=array_filter($ad);$ad=array_keys($ad);if(isset($_GET["grant"]))$ng=array_diff(array_keys(array_filter($De[$Ke],'strlen')),$ad);elseif($Qe==$Fe){$Ne=array_keys((array)$bd[$Ke]);$ng=array_diff($Ne,$ad);$ad=array_diff($ad,$Ne);unset($bd[$Ke]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Ke,$B)&&(!grant("REVOKE",$ng,$B[2]," ON $B[1] FROM $Fe")||!grant("GRANT",$ad,$B[2]," ON $B[1] TO $Fe"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($Qe!=$Fe)queries("DROP USER $Qe");elseif(!isset($_GET["grant"])){foreach($bd
as$Ke=>$ng){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Ke,$B))grant("REVOKE",array_keys($ng),$B[2]," ON $B[1] FROM $Fe");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?lang(230):lang(231)),!$n);if($Db)$g->query("DROP USER $Fe");}}page_header((isset($_GET["host"])?lang(33).": ".h("$ha@$_GET[host]"):lang(138)),$n,array("privileges"=>array('',lang(68))));if($_POST){$K=$_POST;$bd=$De;}else{$K=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Pe;if($Pe!="")$K["hashed"]=true;$bd[(DB==""||$bd?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
as$Ke=>$ad){echo'<th>'.($Ke!="*.*"?"<input name='objects[$s]' value='".h($Ke)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$s]' value='*.*' size='10'>*.*");$s++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>lang(32),"Databases"=>lang(35),"Tables"=>lang(123),"Columns"=>lang(45),"Procedures"=>lang(233),)as$zb=>$Pb){foreach((array)$Of[$zb]as$Nf=>$sb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($sb).'">'.h($Nf);$s=0;foreach($bd
as$Ke=>$ad){$C="'grants[$s][".h(strtoupper($Nf))."]'";$Y=$ad[strtoupper($Nf)];if($zb=="Server Admin"&&$Ke!=(isset($bd["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".lang(234)."<option value='0'".($Y=="0"?" selected":"").">".lang(235)."</select>";else
echo"<td align='center'><label class='block'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Nf=="All privileges"?" id='grants-$s-all'":($Nf=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$s-all');\""))."></label>";$s++;}}}echo"</table>\n",'<p>
<input type="submit" value="',lang(14),'">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="',lang(119),'"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$Ah,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Qd=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$Qd++;}queries_redirect(ME."processlist=",lang(236,$Qd),$Qd||!$_POST["kill"]);}page_header(lang(108),$n);echo'
<form action="" method="post">
...
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$p=fields($a);$Tc=column_foreign_keys($a);$Me="";if($S["Oid"]){$Me=($x=="sqlite"?"rowid":"oid");$w[]=array("type"=>"PRIMARY","columns"=>array($Me));}parse_str($_COOKIE["adminer_import"],$ya);$og=array();$e=array();$rh=null;foreach($p
as$y=>$o){$C=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$C!=""){$e[$y]=html_entity_decode(strip_tags($C),ENT_QUOTES);if(is_shortable($o))$rh=$b->selectLengthProcess();}$og+=$o["privileges"];}list($M,$cd)=$b->selectColumnsProcess($e,$w);$Ed=count($cd)<count($M);$Z=$b->selectSearchProcess($p,$w);$cf=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$Yc=($M?implode(", ",$M):"*".($Me?", $Me":"")).convert_fields($e,$p,$M)."\nFROM ".table($a);$dd=($cd&&$Ed?"\nGROUP BY ".implode(", ",$cd):"").($cf?"\nORDER BY ".implode(", ",$cf):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Sh=>$K){$Ga=convert_field($p[key($K)]);$M=array($Ga?$Ga:idf_escape(key($K)));$Z[]=where_check($Sh,$p);$J=$Xb->select($a,$M,$Z,$M);if($J)echo
reset($J->fetch_row());}exit;}if($_POST&&!$n){$pi=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$eb=array();foreach($_POST["check"]as$bb)$eb[]=where_check($bb,$p);$pi[]="((".implode(") OR (",$eb)."))";}$pi=($pi?"\nWHERE ".implode(" AND ",$pi):"");$Jf=$Uh=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$Jf=array_flip($v["columns"]);$Uh=($M?$Jf:array());break;}}foreach((array)$Uh
as$y=>$X){if(in_array(idf_escape($y),$M))unset($Uh[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Uh===array())$H="SELECT $Yc$pi$dd";else{$Qh=array();foreach($_POST["check"]as$X)$Qh[]="(SELECT".limit($Yc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$dd,1).")";$H=implode(" UNION ALL ",$Qh);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($Z,$Tc)){if($_POST["save"]||$_POST["delete"]){$I=true;$za=0;$O=array();if(!$_POST["delete"]){foreach($e
as$C=>$X){$X=process_input($p[$C]);if($X!==null&&($_POST["clone"]||$X!==false))$O[idf_escape($C)]=($X!==false?$X:idf_escape($C));}}if($_POST["delete"]||$O){if($_POST["clone"])$H="INTO ".table($a)." (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a);if($_POST["all"]||($Uh===array()&&is_array($_POST["check"]))||$Ed){$I=($_POST["delete"]?$Xb->delete($a,$pi):($_POST["clone"]?queries("INSERT $H$pi"):$Xb->update($a,$O,$pi)));$za=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$oi="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$I=($_POST["delete"]?$Xb->delete($a,$oi,1):($_POST["clone"]?queries("INSERT".limit1($H,$oi)):$Xb->update($a,$O,$oi)));if(!$I)break;$za+=$g->affected_rows;}}}$re=lang(240,$za);if($_POST["clone"]&&$I&&$za==1){$Wd=last_id();if($Wd)$re=lang(160," $Wd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$re,$I);if(!$_POST["delete"]){edit_form($a,$p,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(241);else{$I=true;$za=0;foreach($_POST["val"]as$Sh=>$K){$O=array();foreach($K
as$y=>$X){$y=bracket_escape($y,1);$O[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$I=$Xb->update($a,$O," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Sh,$p),!($Ed||$Uh===array())," ");if(!$I)break;$za+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(240,$za),$I);}}elseif(!is_string($Jc=get_file("csv_file",true)))$n=upload_error($Jc);elseif(!preg_match('~~u',$Jc))$n=lang(242);else{cookie("adminer_import","output=".urlencode($ya["output"])."&format=".urlencode($_POST["separator"]));$I=true;$pb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Jc,$je);$za=count($je[0]);$Xb->begin();$Cg=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$L=array();foreach($je[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Cg]*)$Cg~",$X.$Cg,$ke);if(!$y&&!array_diff($ke[1],$pb)){$pb=$ke[1];$za--;}else{$O=array();foreach($ke[1]as$s=>$lb)$O[idf_escape($pb[$s])]=($lb==""&&$p[$pb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$lb))));$L[]=$O;}}$I=(!$L||$Xb->insertUpdate($a,$L,$Jf));if($I)$I=$Xb->commit();queries_redirect(remove_from_uri("page"),lang(243,$za),$I);$Xb->rollback();}}}$dh=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(50).": $dh",$n);$O=null;if(isset($og["insert"])||!support("table")){$O="";foreach((array)$_GET["where"]as$X){if(count($Tc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e&&support("table"))echo"<p class='error'>".lang(244).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$w);$b->selectOrderPrint($cf,$e,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($rh);$b->selectActionPrint($w);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Wc=$g->result(count_rows($a,$Z,$Ed,$cd));$E=floor(max(0,$Wc-1)/$z);}$_g=$M;if(!$_g){$_g[]="*";if($Me)$_g[]=$Me;}$_b=convert_fields($e,$p,$M);if($_b)$_g[]=substr($_b,2);$I=$Xb->select($a,$_g,$Z,$cd,$cf,$z,$E,true);if(!$I)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$E)$I->seek($z*$E);$mc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$x=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last"&&+$z&&$cd&&$Ed&&$x=="sql")$Wc=$g->result(" SELECT FOUND_ROWS()");if(!$L)echo"<p class='message'>".lang(12)."\n";else{$Pa=$b->backwardKeys($a,$dh);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$cd&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(245)."</a>");$Ce=array();$Zc=array();reset($M);$Xf=1;foreach($L[0]as$y=>$X){if($y!=$Me){$X=$_GET["columns"][key($M)];$o=$p[$M?($X?$X["col"]:current($M)):$y];$C=($o?$b->fieldName($o,$Xf):($X["fun"]?"*":$y));if($C!=""){$Xf++;$Ce[$y]=$C;$d=idf_escape($y);$nd=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Pb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($nd.($cf[0]==$d||$cf[0]==$y||(!$cf&&$Ed&&$cd[0]==$d)?$Pb:'')).'">';echo
...
as$K){foreach($K
as$y=>$X)$ce[$y]=max($ce[$y],min(40,strlen(utf8_decode($X))));}}echo($Pa?"<th>".lang(246):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Tc)as$Be=>$K){$Rh=unique_array($L[$Be],$w);if(!$Rh){$Rh=array();foreach($L[$Be]as$y=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$y))$Rh[$y]=$X;}}$Sh="";foreach($Rh
as$y=>$X){if(($x=="sql"||$x=="pgsql")&&strlen($X)>64){$y=(strpos($y,'(')?$y:idf_escape($y));$y="MD5(".($x=='sql'&&preg_match("~^utf8_~",$p[$y]["collation"])?$y:"CONVERT($y USING ".charset($g).")").")";$X=md5($X);}$Sh.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$cd&&$M?"":"<td>".checkbox("check[]",substr($Sh,1),in_array(substr($Sh,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Ed||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Sh)."'>".lang(247)."</a>"));foreach($K
as$y=>$X){if(isset($Ce[$y])){$o=$p[$y];if($X!=""&&(!isset($mc[$y])||$mc[$y]!=""))$mc[$y]=(is_mail($X)?$Ce[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$Sh;if(!$_&&$X!==null){foreach((array)$Tc[$y]as$q){if(count($Tc[$y])==1||end($q["source"])==$y){$_="";foreach($q["source"]as$s=>$Ng)$_.=where_link($s,$q["target"][$s],$L[$Be][$Ng]);$_=($q["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($q["db"]),ME):ME).'select='.urlencode($q["table"]).$_;if($q["ns"])$_=preg_replace('~([?&]ns=)[^&]+~','\\1'.urlencode($q["ns"]),$_);if(count($q["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Rh))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($Rh
as$Kd=>$W)$_.=where_link($s++,$Kd,$W);}$X=select_value($X,$_,$o,$rh);$t=h("val[$Sh][".bracket_escape($y)."]");$Y=$_POST["val"][$Sh][bracket_escape($y)];$hc=!is_array($K[$y])&&is_utf8($X)&&$L[$Be][$y]==$K[$y]&&!$Zc[$y];$qh=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$hc)||$Y!==null){$fd=h($Y!==null?$Y:$K[$y]);echo"<td>".($qh?"<textarea name='$t' cols='30' rows='".(substr_count($K[$y],"\n")+1)."'>$fd</textarea>":"<input name='$t' value='$fd' size='$ce[$y]'>");}else{$ge=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($ge?2:($qh?1:0)).($hc?"":", '".h(lang(248))."'").");\">$X";}}}if($Pa)echo"<td>";$b->backwardKeysPrint($Pa,$L[$Be]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$wc=true;if($_GET["page"]!="last"){if(!+$z)$Wc=count($L);elseif($x!="sql"||!$Ed){$Wc=($Ed?false:found_rows($S,$Z));if($Wc<max(1e4,2*($E+1)*$z))$Wc=reset(slow_query(count_rows($a,$Z,$Ed,$cd)));else$wc=false;}}if(+$z&&($Wc===false||$Wc>$z||$E)){echo"<p class='pages'>";$me=($Wc===false?$E+(count($L)>=$z?2:1):floor(($Wc-1)/$z));if($x!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(249)."', '".($E+1)."'), event); return false;\">".lang(249)."</a>:",pagination(0,$E).($E>5?" ...":"");for($s=max(1,$E-4);$s<min($me,$E+5);$s++)echo
pagination($s,$E);if($me>0){echo($E+5<$me?" ...":""),($wc&&$Wc!==false?pagination($me,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$me'>".lang(250)."</a>");}echo(($Wc===false?count($L)+1:$Wc-$E*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.lang(251).'...\');" class="loadmore">'.lang(252).'</a>':'');}else{echo
lang(249).":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($me>$E?pagination($E+1,$E).($me>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Wc!==false?"(".($wc?"":"~ ").lang(142,$Wc).") ":"");$Ub=($wc?"":"~ ").$Wc;echo
checkbox("all",1,0,lang(253),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Ub' : checked); selectCount('selected2', this.checked || !checked ? '$Ub' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>',lang(245),'</legend><div>
...
';}$Uc=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($Uc['sql']);break;}}if($Uc){print_fieldset("export",lang(70)." <span id='selected2'></span>");$nf=$b->dumpOutput();echo($nf?html_select("output",$nf,$ya["output"])." ":""),html_select("format",$Uc,$ya["format"])," <input type='submit' name='export' value='".lang(70)."'>\n","</div></fieldset>\n";}echo(!$cd&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",lang(69),!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ya["format"],1);echo" <input type='submit' name='import' value='".lang(69)."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($mc,'strlen'),$e);echo"<p><input type='hidden' name='token' value='$Ah'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$P=isset($_GET["status"]);page_header($P?lang(110):lang(109));$hi=($P?show_status():show_variables());if(!$hi)echo"<p class='message'>".lang(12)."\n";else{echo"<table cellspacing='0'>\n";foreach($hi
...
as$y=>$X)json_row("sum-$y",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$m=>$X){json_row("tables-$m",$X);json_row("size-$m",db_size($m));}json_row("");}exit;}else{$jh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($jh&&!$n&&!$_POST["search"]){$I=true;$re="";if($x=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$re=lang(254);}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$re=lang(255);}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$re=lang(256);}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$re=lang(257);}elseif($x!="sql"){$I=($x=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$re=lang(258);}elseif(!$_POST["tables"])$re=lang(9);elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$re.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$re,$I);}page_header(($_GET["ns"]==""?lang(35).": ".h(DB):lang(73).": ".h($_GET["ns"])),$n,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".lang(259)."</h3>\n";$ih=tables_list();if(!$ih)echo"<p class='message'>".lang(9)."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".lang(260)." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".lang(53)."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}$Vb=doc_link(array('sql'=>'show-table-status.html'));echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);" class="jsonly">','<th>'.lang(123),'<td>'.lang(261).doc_link(array('sql'=>'storage-engines.html')),'<td>'.lang(114).doc_link(array('sql'=>'charset-mysql.html')),'<td>'.lang(262).$Vb,'<td>'.lang(263).$Vb,'<td>'.lang(264).$Vb,'<td>'.lang(48).doc_link(array('sql'=>'example-auto-increment.html')),'<td>'.lang(265).$Vb,(support("comment")?'<td>'.lang(47).$Vb:''),"</thead>\n";$T=0;foreach($ih
as$C=>$U){$ki=($U!==null&&!preg_match('~table~i',$U));$t=h("Table-".$C);echo'<tr'.odd().'><td>'.checkbox(($ki?"views[]":"tables[]"),$C,in_array($C,$jh,true),"","formUncheck('check-all');","",$t),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($C)."' title='".lang(40)."' id='$t'>".h($C).'</a>':h($C));if($ki){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.lang(41).'">'.(preg_match('~materialized~i',$U)?lang(121):lang(122)).'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.lang(39).'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",lang(42)),"Index_length"=>array("indexes",lang(125)),"Data_free"=>array("edit",lang(43)),"Auto_increment"=>array("auto_increment=1&create",lang(42)),"Rows"=>array("select",lang(39)),)as$y=>$_){$t=" id='$y-".h($C)."'";echo($_?"<td align='right'>".(support("table")||$y=="Rows"||(support("indexes")&&$y!="Data_length")?"<a href='".h(ME."$_[0]=").urlencode($C)."'$t title='$_[1]'>?</a>":"<span$t>?</span>"):"<td id='$y-".h($C)."'>&nbsp;");}$T++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".lang(238,count($ih)),"<td>".nbsp($x=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$y)echo"<td align='right' id='sum-$y'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$ei="<input type='submit' value='".lang(266)."'".on_help("'VACUUM'")."> ";$Ye="<input type='submit' name='optimize' value='".lang(267)."'".on_help($x=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".lang(118)." <span id='selected'></span></legend><div>".($x=="sqlite"?$ei:($x=="pgsql"?$ei.$Ye:($x=="sql"?"<input type='submit' value='".lang(268)."'".on_help("'ANALYZE TABLE'")."> ".$Ye."<input type='submit' name='check' value='".lang(269)."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".lang(270)."'".on_help("'REPAIR TABLE'")."> ":"")))."<input type='submit' name='truncate' value='".lang(271)."'".confirm().on_help($x=="sqlite"?"'DELETE'":"'TRUNCATE".($x=="pgsql"?"'":" TABLE'"))."> "."<input type='submit' name='drop' value='".lang(119)."'".confirm().on_help("'DROP TABLE'").">\n";$l=(support("scheme")?$b->schemas():$b->databases());if(count($l)!=1&&$x!="sqlite"){$m=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".lang(272).": ",($l?html_select("target",$l,$m):'<input name="target" value="'.h($m).'" autocapitalize="off">')," <input type='submit' name='move' value='".lang(273)."'>",(support("copy")?" <input type='submit' name='copy' value='".lang(274)."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $T);":"")."\">\n";echo"<input type='hidden' name='token' value='$Ah'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.lang(71)."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.lang(195)."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".lang(135)."</h3>\n";$sg=routines();if($sg){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.lang(174).'<td>'.lang(46).'<td>'.lang(212)."<td>&nbsp;</thead>\n";odd('');foreach($sg
as$K){echo'<tr'.odd().'>','<th><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($K["ROUTINE_NAME"]).'">'.h($K["ROUTINE_NAME"]).'</a>','<td>'.h($K["ROUTINE_TYPE"]),'<td>'.h($K["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($K["ROUTINE_NAME"]).'">'.lang(128)."</a>";}echo"</table>\n";}echo'<p class="links">'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.lang(211).'</a>':'').'<a href="'.h(ME).'function=">'.lang(210)."</a>\n";}if(support("sequence")){echo"<h3 id='sequences'>".lang(275)."</h3>\n";$Eg=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema() ORDER BY sequence_name");if($Eg){echo"<table cellspacing='0'>\n","<thead><tr><th>".lang(174)."</thead>\n";odd('');foreach($Eg
